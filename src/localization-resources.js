...
const REPORT_TEMPLATE_KEYS = Object.freeze([
  ...
  reportKey('report.ahr.section.source_understanding', 'Source Understanding', 'heading'),
  reportKey('report.ahr.label.source_type', 'Source type', 'label'),
  reportKey('report.ahr.label.source_understanding_depth', 'Source-understanding depth', 'label'),
  reportKey('report.ahr.label.source_understanding_score', 'Source-understanding score', 'label'),
  reportKey('report.ahr.label.assistant_reference_target', 'Assistant-reference target', 'label'),
  reportKey('report.ahr.section.source_understanding_thesis', 'Thesis And Promise', 'heading'),
  reportKey('report.ahr.section.source_understanding_arc', 'Source Arc', 'heading'),
  reportKey('report.ahr.section.source_understanding_must_not_miss', 'Must-Not-Miss Points', 'heading'),
  reportKey('report.ahr.section.source_understanding_claims', 'Source Understanding Claims', 'heading'),
  reportKey('report.ahr.section.source_understanding_limits', 'Source Understanding Limits', 'heading'),
  reportKey('report.ahr.editorial.scope.source_understanding', 'This synthesis can use a locally derived full-source understanding review for {source_type}, but it does not persist, embed, or transfer the full source text.', 'sentence'),
  reportKey('report.ahr.editorial.scope.source_understanding_insufficient', 'A source-text artifact was supplied, but it did not produce enough local source-understanding material for content review.', 'sentence'),
  reportKey('report.ahr.editorial.source_understanding.thesis', 'The full-source understanding layer frames the artifact this way:', 'sentence'),
  reportKey('report.ahr.editorial.source_understanding.arc', 'The review should follow the source arc rather than only the page-level summary:', 'sentence'),
  reportKey('report.ahr.editorial.source_understanding.must_not_miss', 'The points that should not be missed are:', 'sentence'),
  reportKey('report.ahr.editorial.source_understanding.tension', 'The final review should keep these tensions and limits visible:', 'sentence'),
  reportKey('report.ahr.quality.expected_gap.source_understanding_thin', 'Source understanding was supplied, but its grounded claim or location-reference coverage is limited; content-specific review should stay cautious.', 'sentence'),
  ...
  reportKey('report.ahr.label.grounded_claim_score', 'Grounded claim score', 'label'),
  reportKey('report.ahr.label.useful_recommendation_score', 'Useful recommendation score', 'label'),
...
const REPORT_TEMPLATE_TRANSLATIONS = Object.freeze({
  ja: Object.freeze({
    ...
    'report.ahr.section.source_understanding': '出典理解',
    'report.ahr.label.source_type': '出典種別',
    'report.ahr.label.source_understanding_depth': '出典理解の深さ',
    'report.ahr.label.source_understanding_score': '出典理解スコア',
    'report.ahr.label.assistant_reference_target': 'アシスタント参照目標',
    'report.ahr.section.source_understanding_thesis': '主題と約束',
    'report.ahr.section.source_understanding_arc': '出典の流れ',
    'report.ahr.section.source_understanding_must_not_miss': '見逃し禁止点',
    'report.ahr.section.source_understanding_claims': '出典理解の主張',
    'report.ahr.section.source_understanding_limits': '出典理解の制限',
    'report.ahr.editorial.scope.source_understanding': 'この統括レビューは、{source_type} についてローカルで導出した全文ベースの出典理解レビューを使用できます。ただし、全文は保存、埋め込み、転送されません。',
    'report.ahr.editorial.scope.source_understanding_insufficient': '出典テキスト成果物は提供されましたが、内容レビューを支える十分なローカル出典理解材料を生成できませんでした。',
    'report.ahr.editorial.source_understanding.thesis': '全文理解レイヤーは、この成果物を次のように捉えています:',
    'report.ahr.editorial.source_understanding.arc': 'レビューでは、ページ上の要約だけでなく出典の流れを追う必要があります:',
    'report.ahr.editorial.source_understanding.must_not_miss': '見逃してはいけない点は次の通りです:',
    'report.ahr.editorial.source_understanding.tension': '最終レビューでは、次の揺れや制限を残しておく必要があります:',
    'report.ahr.quality.expected_gap.source_understanding_thin': '出典理解は提供されていますが、根拠付き主張または位置参照カバレッジが限定的です。そのため、内容固有のレビューは慎重に扱う必要があります。',
    ...
    'report.ahr.label.grounded_claim_score': '根拠付き主張スコア',
    'report.ahr.label.useful_recommendation_score': '有用な推奨スコア',

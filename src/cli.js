...
export async function executeCli(argv, context = {}) {
  ...
  try {
    ...
    if (parsed.command === 'agentic review dogfood evidence-pack review-pack') {
      return runtimeResult(parsed.command, await (context.agenticHumanReviewDogfoodEvidencePackReviewPackRunner ?? runAgenticHumanReviewDogfoodEvidencePackReviewPack)(parsed.options, context), parsed.json, now);
    }
...
function usageText(topic) {
  ...
  if (
    ...
    || topic === 'agentic review dogfood evidence-pack review-pack'
    ...
    return [
      ...
      `       ${CLI_NAME} agentic review plan --proposal <proposal> [--review-index <review-artifact-index>] [--provider-effort <provider-native-value>] [--human-baseline <owner-baseline-json>] [--video-evidence <video-evidence-json>] [--content-evidence <content-evidence-json>] [--json]`,
      ...
      `       ${CLI_NAME} agentic review dogfood evidence-pack review-pack --input <workspace-json> [--json]`,
      ...
      '',
      'Plans and runs CLI-only agentic human review. Proposals turn conversational requests into non-executing review intent; planning creates the fresh approval hash and exact transfer flags; provider and dogfood readiness perform no provider call; running requires matching hash, explicit --execute, and exact flags; report quality, benchmark, dogfood planning, dogfood evidence-pack summarization and review-pack projection, calibration, comparison, evidence-set, evidence-set regeneration planning, human-baseline, xhigh, longitudinal, source-text quality, evaluator, and claim-policy commands are read-only advisory checks. For --comparison-kind editorial-quality, --baseline is a workspace-confined reference review text or JSON artifact and the command emits scores only, not the reference or candidate prose. MCP execution remains excluded.'
    ].join('\n');
  }

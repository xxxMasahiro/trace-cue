...
For CLI review, run `target init`, edit the target manifest in the consumer repository, run `target validate`, then run `review --target`. For MCP clients, use `mcp config` output instead of inspecting package files. For Codex, the local plugin bundle is a discovery wrapper around the same CLI/MCP surfaces.

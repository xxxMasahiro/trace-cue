...
- Keep a bounded local browser session open across CLI calls for manual login handoff, same-page actions, observation, checkpoint evidence, and local review handoff when a one-shot visit is not enough.
...
- Import, summarize, and optionally execute Playwright Test regression evidence through a disabled-by-default mode contract, including local artifact import, explicit CLI-only local runs, read-only GitHub Actions artifact retrieval, approved external-CI fetch settings, and read-only non-engineer review-material projection without changing TraceCue review gates.
...
- Attach workspace-confined full text with `agentic review propose|plan --source-text` so Agentic Human Review can derive bounded `source_reading_review` and `source_understanding_review` records for transcripts, articles, PDF-extracted text, meeting notes, documents, and other textual artifacts. TraceCue reads full text locally and composes natural owner-facing prose from bounded understanding metadata, source refs, claims, limitations, and quality diagnostics; full source text and chunk text stay out of JSON, Markdown, provider payloads, receipts, and logs.
- Review an owned or authorized local video with deterministic frame/cadence/cut/A/V/subtitle measurements, a trusted local transcript provider, time-coded cross-modal findings, and a bounded report that keeps measurements separate from advisory evaluations. URL input performs only network-free capability classification and never downloads media.

## Media Review Quickstart

For non-technical use, open the browser dashboard, choose **Video**, leave **Video URL**
selected to check a source's allowed capability without contacting it, or choose
**Local video** for full analysis. The local flow asks for the video, a rights
confirmation, and whether private source data should be removed after review.
...
```bash
trace-cue media source inspect --url 'https://www.youtube.com/watch?v=<id>' --json
...
bounded result is projected. `--retention project-retained` keeps that private
root until an explicit `media review cleanup` with the returned opaque operation
id. A trusted local provider profile is owner-readable and ignored by Git; use
[`templates/media-review-provider-profile.example.json`](templates/media-review-provider-profile.example.json)
as its shape. TraceCue does not install providers, models, FFmpeg, or network
...
Playwright Test regression integration is available as an advisory local surface. It is disabled by default; users can import existing Playwright Test artifacts, explicitly run a local Playwright Test plan from the CLI, fetch exact existing GitHub Actions artifacts through read-only `gh run` commands, approve external-CI settings that resolve the latest matching successful run before an explicit fetch, and project normalized results into non-engineer review cards plus standard/deep/xhigh review inputs. The browser dashboard exposes mode selection, import, approved external-CI settings, external CI artifact fetch, and compact review material, but not local browser execution.
...
Agentic Human Review quality evaluation now also has read-only local commands for the dogfood phase after a manual run. Use human-baseline registry/overlay/draft/approval/validate/compare/claim-readiness, evidence-set validation/summary/regeneration planning, dogfood evidence-pack summarization and review-pack projection, batch comparison, evaluator policy, xhigh planning/simulation, editorial-quality comparison, source-text effort-matrix verification, longitudinal quality, claim policy/audit, and claim standard-gate commands to compare standard/deep/xhigh runs and benchmark cases before making any quality claim. AI drafts remain preparation only; approved baselines require owner approval metadata, synthetic or fixture-only markers do not verify as owner evidence, and owner-label comparison scoring requires structured findings with local evidence references. The `editorial-quality` comparison kind accepts a workspace-confined reference review text or JSON artifact as the baseline, compares it to a candidate advisory result's editorial synthesis, emits only hashes, scores, deltas, and diagnostics, and never outputs the reference prose or candidate review body. `agentic review quality source-text` accepts standard, deep, and xhigh source-text result artifacts, checks source-understanding completion, effort-specific editorial output differences, xhigh critique signals, and full-source/chunk-text non-persistence, and optionally compares each effort to a workspace-confined reference review without outputting reference or candidate prose. `agentic review dogfood evidence-pack summarize` accepts a workspace-confined dogfood evidence-pack or evidence-set manifest and emits a compact matrix, owner-review, claim-review, and regeneration-handoff summary without detailed result paths, raw provider responses, credentials, concrete rerun commands, provider calls, artifact writes, or MCP exposure. `agentic review dogfood evidence-pack review-pack` reuses the same evidence-pack summary, claim-readiness, longitudinal-quality, and claim-standard-gate data to emit an owner-facing status, standard/deep/xhigh matrix, grouped blockers, top owner actions, trust/safety strip, and pathless advanced references. These commands read local result metadata only; they do not approve live provider execution and do not permit human-equivalent or human-superior claims by themselves. The claim standard gate returns a failed command status when owner claim-review readiness does not pass, and the evidence-set regeneration planner can convert those blockers into dependency-ordered command templates, but neither command mutates deterministic release gates or runs provider reruns. Provider-rerun templates are filled only after the planner validates approved plan paths, plan hashes, exact transfer flags, exact run commands, and target case/effort alignment from an explicit registry row or the result -> execution -> plan artifact chain. Proof-readiness diagnostics must show the full benchmark-case by effort matrix, missing case-level `direct-vs-tracecue` comparisons, weak calibration blockers, and adapter structured-output gaps instead of relying on total result counts.
...
node ./bin/trace-cue.js session act --session <id> --action '{"type":"click","selector":"text=Save"}' --json
...
node ./bin/trace-cue.js agentic review quality source-text --standard <standard-result> --deep <deep-result> --xhigh <xhigh-result> --reference-review <workspace-text-or-json> --json
node ./bin/trace-cue.js agentic review dogfood evidence-pack summarize --input <dogfood-evidence-pack-or-evidence-set> --json
node ./bin/trace-cue.js agentic review dogfood evidence-pack review-pack --input <dogfood-evidence-pack-or-evidence-set> --json
node ./bin/trace-cue.js agentic review claim standard-gate --evidence-set <agentic-evidence-set> --json
...
- `full`: current compatibility surface for local observe/review/target workflows plus bounded process-scoped supervise. No-profile `trace-cue-mcp` and the packaged `.mcp.json` resolve to this profile.
- `admin`: explicit reserved local-maintenance profile. In this phase it exposes the approved stdio `agent execution plan/run` bridge and admin-only persistent session tools beyond `full`; it does not expose cleanup execution, unrelated provider/API execution, HTTP `full` or `admin`, socket transport, shell tools, external upload, existing-browser-profile reuse, provider credentials, or arbitrary process control.

...
- Import, summarize, and optionally execute Playwright Test regression evidence through a disabled-by-default mode contract, including local artifact import, explicit CLI-only local runs, read-only GitHub Actions artifact retrieval, and approved external-CI fetch settings without changing TraceCue review gates.
...
Playwright Test regression integration is available as an advisory local surface. It is disabled by default; users can import existing Playwright Test artifacts, explicitly run a local Playwright Test plan from the CLI, fetch exact existing GitHub Actions artifacts through read-only `gh run` commands, or approve external-CI settings that resolve the latest matching successful run before an explicit fetch. The browser dashboard exposes mode selection, import, approved external-CI settings, and external CI artifact fetch, but not local browser execution.

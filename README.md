...
| MCP stdio | Run `browser-debug-mcp` or use the packaged `.mcp.json`. | MCP clients that prefer stdio tools. | No-profile and `.mcp.json` preserve compatibility by resolving to `full`; low-trust clients should pass `--profile safe`. |
| HTTP MCP safe | Run `browser-debug-mcp --transport http --profile safe --host 127.0.0.1 --port 0`. | MCP-compatible local tooling that needs an HTTP endpoint instead of stdio. | This transport is safe-profile-only, loopback-only, bearer-token gated, and limited to one JSON-RPC request per POST in this phase. |

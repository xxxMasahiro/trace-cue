...
- `safe`: no-browser/no-delete/no-provider tool surface for discovery, schemas, target validation, resource status, artifact planning, read-only local agent advisory/status inspection, read-only visual review dashboard inspection, read-only capture planning, read-only MCP execution gate inspection, and MCP capability policy inspection.
...
`trace-cue mcp capabilities --json` reports the current profile tool surfaces, supported transports, and explicit operations that are still excluded from MCP, including visual review result preparation, visual review execution, visual review aggregation, desktop provider-preparation planning, and raw-pixel visual provider execution. Safe MCP includes read-only visual review dashboard, capture planning, and MCP execution gate inspection only. It is read-only, does not start a server, and confirms that `admin` currently remains equivalent to `full` rather than enabling cleanup execution, provider/API execution, `agent execution run`, shell tools, daemon/session control, or credential-bearing workflows.

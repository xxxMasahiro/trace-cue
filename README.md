...
- Import, summarize, and optionally execute Playwright Test regression evidence through a disabled-by-default mode contract, including local artifact import, explicit CLI-only local runs, and read-only GitHub Actions artifact retrieval without changing TraceCue review gates.
...
Playwright Test regression integration is available as an advisory local surface. It is disabled by default; users can import existing Playwright Test artifacts, explicitly run a local Playwright Test plan from the CLI, or fetch existing GitHub Actions artifacts through read-only `gh run` commands. The browser dashboard exposes mode selection, import, and external CI artifact fetch, but not local browser execution.

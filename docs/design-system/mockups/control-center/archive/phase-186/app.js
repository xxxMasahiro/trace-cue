...
function renderAiSetupDialog() {
  const aiIdentity = currentAiIdentity();
  const disconnect = state.aiConnectionKind === "api"
    ? '<button class="button text" type="button" data-action="disconnect-ai">接続を解除</button>'
    : '';
  const current = state.aiConnected ? `<div class="ai-setup-current"><span aria-hidden="true">✓</span><div><strong>接続済み</strong><p>${aiIdentity.name}</p></div>${disconnect}</div>` : "";

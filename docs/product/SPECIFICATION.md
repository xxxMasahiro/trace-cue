...
`reviewBrief` can declare `summary`, `userRoles`, and `decisionNeeds[]`. `rubric[]` can declare `id`, `category`, optional `pageId`, optional `selector`, `criterion`, `expectedEvidence`, `matchMode`, `textMatch`, `severity`, and `required`. Manifest pages can declare a generic `role` such as `workflow_overview`, `triage_detail`, or `release_gate`. The local advisory evaluates decision needs and rubric criteria against reviewed page text or selector-scoped element text, but it does not copy expected evidence values or source values into report output.
...
`content_ux_findings.category` can include `content_contract`, `source_data_alignment`, `workflow_state_clarity`, `information_architecture`, `next_action_clarity`, `navigation_clarity`, `coverage_contract`, or `review_scope`. `content_ux_page_handoff` groups advisory findings by manifest page, including page status, finding count, top categories, and bounded top findings. `content_ux_manifest_authoring` suggests manifest-only improvements such as declaring audience or goal, adding inline source data, adding page data bindings, adding user questions, fixing binding pointers, or strengthening next-action and navigation contracts. These suggestions are advisory and never mutate the manifest automatically.

...
## Agentic Human Review Video Evidence Intake And Localized Report Chrome

- Purpose: let Agentic Human Review distinguish page-only review from page-plus-video-evidence review while keeping video analysis outside TraceCue and making TraceCue-owned report labels follow artifact output language settings.
- Implemented scope: accept optional `--video-evidence <workspace-json>` for `agentic review propose` and `agentic review plan`; normalize a tool-neutral metadata-only video evidence artifact; reject raw video, raw audio, frame, image, binary, base64, data URI, payload, and full-transcript fields; carry path-free video evidence summaries into proposals, plans, packages, provider payload filters, advisory results, schemas, and editorial synthesis; expose an `evidence_scope` record; allow provider payloads to receive video summaries only under existing page-text transfer approval while stripping local paths, source URLs, and raw media; and resolve TraceCue-owned Agentic Human Review report headings, labels, fallback sentences, and evidence-scope lines through local report templates for the supported locale contract.
- Non-scope: implementing a video analyzer, changing FrameCue, fetching or downloading remote video, analyzing raw video inside TraceCue, sending raw media or full transcripts to providers, adding provider/API calls, adding MCP tools, translating provider-authored text, changing report-quality metrics, satisfying owner-baseline or benchmark proof, authorizing claim readiness, mutating deterministic findings, changing release gates, or adding target-specific branches for any site, repository, URL, provider, model, or analyzer.
- Implementation order: add the schema and parser option; add workspace-confined video evidence loading and redaction; integrate source summaries into proposal, plan, package, provider filter, advisory normalization, and editorial synthesis; add localized report template keys and Japanese coverage for TraceCue-owned fixed text; add focused no-browser and packed-install tests for accepted metadata, raw-media rejection, provider payload stripping, evidence scope, source references, and localized Markdown chrome; synchronize product/security/verification/workflow docs and manifests; run syntax checks, focused tests, full local tests, docs/security checks, product gate, and diff checks.
- Recovery: this slice is additive, metadata-only, and fail-closed. Existing proposals, plans, packages, advisory results, provider payloads, and Markdown reports remain valid when no video evidence is supplied. Rollback is a standard Git revert without artifact migration, credential cleanup, provider-policy change, MCP-policy change, or remote-service change.

...
- Implemented scope: accept optional `--content-evidence <workspace-json>` for `agentic review propose` and `agentic review plan`; keep `--video-evidence <workspace-json>` as a compatibility input; normalize analyzer-neutral bounded content evidence with source type, summaries, content units, claims, limitations, coverage, and provenance; reject raw media, raw binaries, base64/blob/data URI payloads, raw HTML/PDF bytes, full documents, and full transcripts; carry path-free supplemental content evidence into proposals, plans, packages, provider payload filters, advisory results, report-quality diagnostics, schemas, Markdown reports, and editorial synthesis; expose additive `content_evidence_*` fields on `evidence_scope`; allow provider payloads to receive bounded content evidence only under existing page-text transfer approval while stripping local paths and source locators; and resolve TraceCue-owned report headings, labels, fallback sentences, content-evidence labels, and evidence-scope lines through local report templates for the supported locale contract.
...
- Implementation order: add the schema and parser option; add workspace-confined content evidence loading and redaction; keep video evidence compatibility by projecting video summaries into generic supplemental content evidence; integrate source summaries and bounded units into proposal, plan, package, provider filter, advisory normalization, report-quality, Markdown report, and editorial synthesis; add localized report template keys for content evidence and xhigh stance; add focused no-browser and packed-install tests for accepted metadata, raw-payload rejection, provider payload stripping, evidence scope, source references, report-quality diagnostics, and localized Markdown chrome; synchronize product/security/verification/workflow docs and manifests; run syntax checks, focused tests, full local tests, docs/security checks, product gate, and diff checks.

...
preserving CLI capability, read-model fields, bounded actions, and security
boundaries.
...
Implemented scope: add disabled-by-default `playwright-test` mode settings for `disabled`, `import_only`, `local_run`, and `external_ci`; add local result import from workspace-confined JSON/JUnit/HTML-reference artifacts; add read-only normalized E2E result review-material projection with non-engineer review cards, evidence-quality limits, optional baseline comparison, and standard/deep/xhigh review-input blocks; add CLI-only local-run planning and execution with fixed Playwright CLI invocation, plan hash matching, timeout handling, receipts, and redaction; add read-only GitHub Actions artifact listing/viewing/downloading through `gh run list/view/download` only; add approved external-CI fetch settings that resolve the latest matching successful run before explicit download; scan downloaded artifacts before import; add Control Center Regression and Settings views for mode selection, local import, review material, approved external-CI settings, and external CI artifact fetch without exposing local browser execution; add CLI/API/schema registry/public schema coverage, product-local design-system component metadata, tests, docs, and manifests.

Non-scope: remote CI trigger, rerun, cancel, workflow dispatch, PR/status/comment writes, unapproved or ambiguous GitHub artifact auto-discovery without exact artifact selection, fuzzy artifact matching, raw artifact viewers for review material, MCP exposure, Control Center local browser execution, provider/API review, deterministic TraceCue finding mutation, Agentic Human Review proof mutation, release-gate mutation, default browser launches, hard-coded repository paths, and existing-feature tradeoffs.

Implementation order: add shared mode/result/artifact helpers; add import, local-run, external-ci, regression summary, and E2E result review-material projection modules; wire parser, CLI help, constants, API exports, schemas, and schema registry; expose bounded Control Center actions and React/Vite Regression/Settings UI through the existing loopback server and dev proxy, with review material derived in the read model before rendering; synchronize design-system component metadata, product/workflow docs, security, verification, manifests, package tests, and structure gates; run focused Playwright Test tests, Control Center tests, architecture tests, full no-browser tests, package checks, product docs/security/structure/design/CI checks, product gate, `git diff --check`, commit, push, confirm main CI, fetch, and verify local/remote sync.
...
Implemented scope: add `agentic review dogfood evidence-pack summarize` and `agentic review dogfood evidence-pack review-pack`; add CLI/API/schema exports; reuse evidence-set summary, human-baseline claim-readiness, longitudinal-quality, claim-standard-gate, and owner-review-context logic; emit matrix completion, missing case-by-effort cells, blocker categories, owner-review digest, claim-review state, advisory regeneration handoff, owner-facing status, standard/deep/xhigh matrix badges, grouped blockers, top owner actions, trust/safety flags, and pathless advanced references; suppress detailed result paths, source paths, raw provider responses, credential values, full source text, chunk text, candidate/reference prose, concrete rerun commands, and raw hash values from owner-facing review packs; keep provider calls, API calls, credential reads, external transfer, artifact writes, browser launch, MCP exposure, automatic reruns, proof claims, release-gate mutation, and human-equivalent or human-superior claim authorization out of scope.

Implementation order: add parser/API/CLI command routing; add read-only dogfood evidence-pack input resolution, shared projection preparation, summary builder, and review-pack builder beside existing Agentic Human Review evidence-set code; register the public schemas and package discovery; extend no-browser CLI coverage for manifest input, direct evidence-set input, ready/incomplete review-pack status, duplicate/missing matrix blockers, unsupported execution/provider options, workspace confinement, forged downstream owner-context sanitization, leak suppression, and schema parity; extend packed-install API/schema coverage; synchronize product, workflow, security, verification, changelog, README, and manifest records; run syntax checks, focused no-browser tests, package smoke, product docs/security/structure checks, product gate, and `git diff --check`; then commit, push, monitor remote CI, and synchronize local/remote branch state.

Recovery: this slice is additive and local. Existing Agentic Human Review proposal, plan, run, readiness, report-quality, source-text quality, evidence-set, human-baseline, comparison, evaluator policy, xhigh, longitudinal quality, claim diagnostics, evidence regeneration planning, Responses adapter, visual review, agent execution, MCP profiles, safe HTTP MCP, deterministic findings, and release gates remain compatible. Rollback is a standard Git revert of the dogfood evidence-pack summary/review-pack commands, schemas, docs, and focused tests; no artifact migration, credential cleanup, provider-policy change, MCP-policy change, release-gate change, or claim-standard policy change is involved.
...
Implemented scope: add `agentic review quality source-text`; require workspace-confined `--standard`, `--deep`, and `--xhigh` Agentic Human Review result artifacts; optionally accept a workspace-confined `--reference-review`; validate each advisory result; summarize source-text metadata, bounded same-source identity invariants, source-reading/source-understanding completion, editorial synthesis hashes, quality scores, pairwise effort deltas, xhigh critique/evidence-limit/conclusion-change readiness, optional reference-review comparison summaries, output-safety diagnostics, schema/API/package coverage, parser/help coverage, and product docs; keep full source text, chunk text, direct raw source-text aliases, candidate full reviews, reference prose, private source identity values, result paths, source locators, source titles, raw provider responses, credentials, provider execution, MCP exposure, proof-contract satisfaction, release-gate mutation, and human-equivalent or human-superior claim authorization out of scope.

Implementation order: synchronize product/workflow/security/verification docs; add parser/API/CLI command routing; add read-only source-text quality builder and schema; extend no-browser CLI coverage with standard/deep/xhigh source-text results, same-source identity invariants, raw source/chunk alias diagnostics, output-safety checks, and optional reference review; extend packed-install schema/API coverage when schema/package contracts change; run syntax checks, focused no-browser tests, product docs/security/structure checks, package checks, product gate, and `git diff --check`; then commit, push, monitor remote CI, and synchronize local/remote branch state.
...
Implemented scope: add `--source-text` to Agentic Human Review proposal and plan; normalize plain-text and JSON source-text exports; reject raw media, raw binaries, base64/blob/data URI payloads, credential-bearing fields, and raw/full structured payload fields; chunk and hash the text locally without persisting full source text or chunk text; derive an effort-aware `source_reading_review` with narrative flow, key points, concrete examples, tensions, cautions, recommended direction, excerpt refs, quality targets, and a natural review seed; derive an effort-aware `source_understanding_review` with thesis, audience promise, narrative arc, turning points, repeated motifs, must-not-miss points, limitations, reviewer implications, evidence claims, assistant-reference quality target, and coverage metrics; add an `editorial_integrator` record so editorial synthesis can use full-source understanding first and cross-check it against existing TraceCue findings, role opinions, owner-baseline findings, content evidence, video evidence, quality signals, and recommendations; build a target-independent editorial narrative plan from source-understanding signals and existing normalized AHR findings, reader-impact records, role opinions, and action-plan signals; compose the final review as natural owner-facing prose while keeping fake-provider scaffolding, `Step`/`role` markers, operational effort labels, assistant-reference target labels, duplicate source anchors, target-specific heuristics, and boundary boilerplate out of the prose body; make the source-understanding composer effort-profiled so `standard` remains practical, `deep` adds audience/prioritization detail, and complete `xhigh` surfaces critique, verification, counterpoints, evidence limits, and conclusion-change conditions; include source-text natural-composition matrix coverage for standard/deep/xhigh across video, web page, PDF, meeting notes, and document source types; include bounded source-understanding review data in generic provider payloads under `allow-page-text` while compacting source refs to ids and hashes without excerpt text or source locators; include source-text metadata, source-reading review, source-understanding review, editorial integration, source-understanding report-quality diagnostics, Markdown report sections, schemas, package smoke coverage, and CLI tests; and keep deterministic findings, release gates, MCP exposure, provider approval, owner-baseline proof, benchmark proof, equality claims, and superiority claims unchanged.

Non-scope: implementing FrameCue, downloading remote pages, reading raw videos, OCR, PDF binary parsing, translating source text, storing full transcripts or full documents, sending full source text to providers, adding provider calls, changing report-quality proof scores into claim approval, weakening content-evidence raw-content rejection, exposing Agentic Human Review through MCP, mutating deterministic review artifacts, changing release gates, parent-repository changes, consumer-repository changes, dependency additions, package publication, or target-specific branches.

Implementation order: synchronize product/workflow/security/verification docs; add parser options and source-text normalization; add source-reading reviewer output and effort-depth behavior; add source-understanding reviewer output and assistant-reference target metadata; integrate source reading and source understanding into proposal, plan, review package, provider filters, adapter compact payload, result normalization, editorial synthesis, editorial integrator output, Markdown reports, report-quality diagnostics, schemas, schema registry, package-smoke coverage, and focused CLI tests; run syntax checks, full no-browser CLI tests, product docs/security checks, package checks, product gate, and `git diff --check`; then commit, push, monitor main CI, and synchronize local and remote.

Recovery: this slice is additive and local. Existing Agentic Human Review proposal, plan, run, status/list, report-quality, content evidence, video evidence, staged execution, owner-baseline lifecycle, benchmark, compare, evidence-set, claim diagnostics, Responses adapter, visual review, agent execution, MCP profiles, safe HTTP MCP, persistent sessions, release readiness, artifact-root policy, alias compatibility, deterministic findings, and release gates remain compatible. Rollback is a standard Git revert of the source-text option, source-reading helpers, docs, schemas, and tests; no artifact migration, credential storage, raw response persistence, release-gate change, provider-call authorization expansion, or MCP permission expansion is involved.

### Agentic Human Review Coverage Patch Repair

Purpose: recover reusable real-provider dogfood when a provider returns a substantively complete advisory but the final full replacement repair still omits required benchmark coverage rows. The slice stays generic by asking the provider for a missing-only coverage patch and by keeping TraceCue's existing fail-closed post-validation as the authority.

Implemented scope: detect coverage-only failures after at least one full repair attempt; build one bounded provider-authored coverage patch request with redacted prior-advisory context, `coverage_repair_targets`, required coverage templates, and allowed evidence-reference ids; accept only `benchmark_requirement_coverage` patch output; merge only exact missing rows with catalog-backed evidence references; record provider patch provenance; and re-run the same benchmark, owner-baseline, claim, and effort validation before returning success. Non-scope: synthesizing required mentions or dimensions from prose, owner-baseline findings, target URLs, benchmark case literals, local artifacts, target-specific branches, relaxed thresholds, or baseline prose; repairing invalid existing rows with unknown evidence references; increasing full replacement repair attempts; storing raw provider responses or credentials; exposing Agentic Human Review through MCP; mutating deterministic findings; changing release gates; parent-repository changes; consumer-repository changes; dependency additions; package publication; or CI-default live provider calls.

Implementation order: synchronize product and workflow documents; add coverage-only eligibility and missing-row patch request construction; add exact-row merge and provenance diagnostics; add focused no-browser adapter tests for successful provider-authored patch repair and rejected mismatched/unknown-evidence patch rows; run syntax checks, focused Responses adapter tests, full no-browser tests, product docs/security checks, product gate, and `git diff --check`; restart the loopback adapter so approved real-provider dogfood uses the repaired adapter.

Recovery: this slice is additive and local. Existing Agentic Human Review proposal, plan, run, status/list, report-quality, benchmark, calibrate, compare, evidence-set, human-baseline lifecycle, xhigh, claim-readiness, longitudinal quality, claim-standard-gate, full replacement repair, forbidden-claim absence completion, owner-baseline proof validation, staged execution, visual review, agent execution, MCP profiles, safe HTTP MCP, release readiness, artifact-root policy, alias compatibility, deterministic findings, and release gates remain compatible. Rollback is a standard Git revert of the patch-repair helper, docs, and focused tests; no artifact migration, publication, credential storage, raw response persistence, release-gate change, provider-call authorization expansion, or MCP permission expansion is involved.

### Agentic Human Review Provider Output JSON Wrapper Recovery

Purpose: recover reusable real-provider dogfood when a provider returns one valid advisory JSON object wrapped in Markdown fences, leading or trailing prose, or JSON-string encoding despite the raw-JSON instruction. The slice stays generic by parsing only provider-authored JSON candidates in memory and by keeping the existing TraceCue post-validation contract as the authority.

Implemented scope: replace direct provider `output_text` parsing with a bounded helper that tries direct JSON first, decodes JSON-string wrappers with a strict recursion limit, accepts one JSON Markdown fence, extracts one balanced JSON object from prose-wrapped output, and rejects malformed, ambiguous, multiple, array, primitive, non-JSON-fenced, or non-advisory output. Strengthen normal and repair instructions to request one raw JSON object, keep repair attempts unchanged, and add focused adapter parser and repair-path regressions. Non-scope: repairing malformed JSON by guessing, synthesizing advisory content, accepting multiple candidates, relaxing benchmark, owner-baseline, xhigh, claim, or evidence-reference validation, storing raw provider output, recording credentials, exposing Agentic Human Review through MCP, mutating deterministic findings, changing release gates, parent-repository changes, consumer-repository changes, dependency additions, package publication, or CI-default live provider calls.

Implementation order: synchronize product and workflow documents; add the bounded provider-output JSON extraction helper; strengthen raw-output instructions for initial and repair calls; add focused no-browser parser and repair-path tests; run syntax checks, focused Responses adapter tests, full no-browser tests, product docs/security checks, product gate, and `git diff --check`; restart the loopback adapter so approved real-provider dogfood uses the repaired adapter.

Recovery: this slice is additive and local. Existing Agentic Human Review proposal, plan, run, status/list, report-quality, benchmark, calibrate, compare, evidence-set, human-baseline lifecycle, xhigh, claim-readiness, longitudinal quality, claim-standard-gate, forbidden-claim absence completion, owner-baseline proof validation, staged execution, visual review, agent execution, MCP profiles, safe HTTP MCP, release readiness, artifact-root policy, alias compatibility, deterministic findings, and release gates remain compatible. Rollback is a standard Git revert of the parsing helper, prompt wording, docs, and focused tests; no artifact migration, publication, credential storage, raw response persistence, release-gate change, provider-call authorization expansion, or MCP permission expansion is involved.

### Agentic Human Review Forbidden Claim Absence Completion

Purpose: recover reusable real-provider dogfood when a provider satisfies the substantive review but repeatedly omits required forbidden-claim absence rows from `benchmark_requirement_coverage`. The slice stays generic by deriving obligations from the active benchmark/owner-baseline coverage templates, not from target URLs, benchmark case literals, repository names, or page-specific text.

Implemented scope: require nested benchmark coverage arrays in the Responses adapter provider JSON schema whenever effective coverage is active; derive nested `minItems` from the required coverage templates; add path-oriented `coverage_repair_targets` to bounded repair context; after at least one provider repair attempt, complete only missing forbidden-claim absence rows when the provider advisory does not assert the forbidden claim and a catalog-backed evidence reference is available; mark completed rows as adapter-derived and advisory-only; and add focused no-browser adapter regressions for missing forbidden absence rows and coverage-style forbidden checks. Non-scope: synthesizing required mentions, required dimensions, owner-baseline findings, or prose-derived coverage; increasing repair attempts; accepting unknown evidence references; relaxing forbidden-claim semantics; authorizing human-equivalent or human-superior claims; storing raw provider responses or credentials; exposing Agentic Human Review through MCP; mutating deterministic findings; changing release gates; parent-repository changes; consumer-repository changes; dependency additions; package publication; or CI-default live provider calls.

Implementation order: synchronize product and workflow documents; strengthen provider schema and repair-target metadata; add repair-after-attempt forbidden-claim absence completion with adapter-derived provenance; add focused adapter tests; run syntax checks, focused Responses adapter tests, full no-browser tests, product docs/security checks, and product gate; restart the loopback adapter so approved real-provider dogfood uses the repaired adapter.

Recovery: this slice is additive and local. Existing Agentic Human Review proposal, plan, run, status/list, report-quality, benchmark, calibrate, compare, evidence-set, human-baseline lifecycle, xhigh, claim-readiness, longitudinal quality, claim-standard-gate, Responses adapter startup, visual review, agent execution, MCP profiles, safe HTTP MCP, release readiness, artifact-root policy, alias compatibility, deterministic findings, and release gates remain compatible. Rollback is a standard Git revert of the schema/repair/completion changes, docs, and focused tests; no artifact migration, publication, credential storage, raw response persistence, release-gate change, provider-call authorization expansion, or MCP permission expansion is involved.
...
Implemented scope: treat placeholder role output consistently when it is marked by a truthy `placeholder_generated` value, `reported_by_provider=false`, or role-output did-not-return/not-available/missing-output summary wording; exclude those records from reported role/round coverage; preserve ordinary content findings about unavailable page data as reported role output; expose bounded `placeholder_outputs` diagnostics with stage id, stage kind, role, round, reason, and planned-role match in adapter failure details and repair context; instruct repair retries to replace exact placeholder role outputs; and add focused no-browser adapter coverage for summary-placeholder staged repair, content not-available wording, and truthy placeholder flags. Non-scope: accepting placeholder output as proof, increasing default repair attempts, synthesizing role opinions, relaxing owner-baseline or benchmark validation, storing raw provider responses, storing provider request payloads, recording credential values, adding target-specific branches, exposing Agentic Human Review through MCP, mutating deterministic findings, changing release gates, parent-repository changes, consumer-repository changes, dependency additions, or package publication.

Implementation order: update product and workflow documents; add reusable placeholder role-output classification; thread safe placeholder metadata through aggregate contract diagnostics and repair context; extend repair instructions; add focused adapter regression coverage; run focused no-browser tests, product document checks, product security checks, and the product gate; then resume approved staged real-provider dogfood from the previously blocked owner-contract cell.

Recovery: this slice is additive and local. Existing Agentic Human Review proposal, plan, one-shot run, staged run, status/list, report-quality, benchmark, calibrate, compare, evidence-set, human-baseline lifecycle, xhigh planning/simulation, claim-readiness, longitudinal quality, claim-standard-gate, evidence regeneration planning, Responses adapter startup, visual review, agent execution, MCP profiles, safe HTTP MCP, release readiness, artifact-root policy, alias compatibility, deterministic findings, and release gates remain compatible. Rollback is a standard Git revert of the placeholder diagnostics, docs, and focused tests; no artifact migration, publication, credential storage, raw response persistence, release-gate change, provider-call authorization expansion, or MCP permission expansion is involved.

### Agentic Human Review Adapter Claim Candidate Filtering

Purpose: keep real-provider dogfood runs recoverable when a provider returns optional `review_claims` that are structurally unsupported, placeholder-like, or worded as human-equivalent or human-superior claims. The slice stays generic across targets, repositories, URLs, providers, models, efforts, and artifact layouts by applying the same mechanical claim-candidate filter to provider output before TraceCue result normalization.

Implemented scope: filter adapter-returned claim candidates to retain only non-placeholder claims with catalog-backed evidence references or valid planned/prior-stage role support; reject equality, superiority, unsupported, and unknown-role claim candidates with bounded diagnostics; include `adapter_claim_filtering` in adapter responses and advisory results; carry rejected candidates into `claim_integrity` so completed advisory runs remain usable but claim-numerator eligibility stays false; let staged final synthesis use prior normalized stage roles as claim support; and add focused no-browser coverage for direct provider output, Responses adapter output, staged role support, and proof-ready matrix fixtures. Non-scope: retrying providers solely for optional claim candidates, synthesizing replacement claims, authorizing human-equivalent or human-superior claims, weakening owner-baseline or benchmark validation, storing raw provider responses, recording credential values, exposing Agentic Human Review through MCP, mutating deterministic findings, changing release gates, parent-repository changes, consumer-repository changes, dependency additions, package publication, or target-specific dogfood branches.

Implementation order: synchronize product and workflow documents; add reusable adapter claim-candidate sanitization; add result-level claim-filtering normalization and equality/superiority rejection; preserve strict claim-numerator exclusion through `claim_integrity`; extend focused CLI and adapter tests; run focused tests, full no-browser tests, product docs/security checks, package checks, and product gate; then resume approved real-provider dogfood reruns from the previously failed cell without retrying provider calls that only failed due optional claim-candidate shape.

Recovery: this slice is additive and local. Existing Agentic Human Review proposal, plan, one-shot run, staged run, status/list, report-quality, benchmark, calibrate, compare, evidence-set, human-baseline lifecycle, xhigh, claim-readiness, longitudinal quality, claim-standard-gate, evidence regeneration planning, Responses adapter startup, visual review, agent execution, MCP profiles, safe HTTP MCP, release readiness, artifact-root policy, alias compatibility, deterministic findings, and release gates remain compatible. Rollback is a standard Git revert of the claim filtering, docs, and focused tests; no artifact migration, publication, credential storage, raw response persistence, release-gate change, provider-call authorization expansion, or MCP permission expansion is involved.

### Agentic Human Review Staged Effort Provider Execution

Purpose: make approved real-provider `standard`, `deep`, and `xhigh` dogfood recover from structural one-shot provider timeouts without relying on a target-specific retry, longer timeout, hard-coded URL, hard-coded model, or weaker validation. The slice remains generic by using the existing plan hash, transfer flags, provider capability hash, owner-baseline requirement contract, benchmark contract, and effort role/round plan as the execution authority.

Implemented scope: add an explicit `agentic review run --execution-mode staged` path for approved `standard`, `deep`, and `xhigh` plans; preserve the default one-shot run path; split provider execution into bounded staged calls that keep env-only credentials, no raw provider response storage, no credential value recording, no MCP exposure, advisory-only output, and exact transfer boundaries; record normalized non-final stage metadata separately from final proof evidence; deterministically aggregate staged output into the existing `agentic_human_review_advisory` result contract; record true multi-call execution separately from effort completion, and mark xhigh complete or claim-ready only when the final normalized result satisfies the existing role, round, critique, verification, synthesis, benchmark, owner-baseline, and claim-integrity contracts. Non-scope: changing timeout defaults, treating stage output as final evidence, accepting incomplete staged output as proof-ready, authorizing human-equivalent or human-superior claims, exposing Agentic Human Review through MCP, storing raw provider responses or credentials, mutating deterministic findings, changing release gates, parent-repository changes, consumer-repository changes, dependency additions, package publication, or target-specific dogfood branches.

Implementation order: synchronize product and workflow documents; keep parser/API support for `--execution-mode staged` while preserving the one-shot default; generalize reusable staged effort contract helpers and fail-closed validation for unsupported efforts; implement staged execution for fake/injected/API providers through the existing provider and loopback adapter boundaries; add deterministic aggregation and execution/receipt metadata; extend focused no-browser tests for standard/deep/xhigh staged success, incomplete stage failure semantics, plan/transfer drift rejection before fetch, adapter no-store/no-tools boundaries, and claim-state non-authorization; run syntax checks, focused Agentic Human Review tests, product docs/security checks, package smoke checks, and product gate; then rerun the approved real-page staged dogfood cells only after local verification passes.

Recovery: this slice is additive and local. Existing Agentic Human Review proposal, plan, default run, status/list, report-quality, benchmark, calibrate, compare, evidence-set, human-baseline lifecycle, xhigh planning/simulation, claim-readiness, longitudinal quality, claim-standard-gate, evidence regeneration planning, Responses adapter startup, visual review, agent execution, MCP profiles, safe HTTP MCP, release readiness, artifact-root policy, alias compatibility, deterministic findings, and release gates remain compatible. Rollback is a standard Git revert of the staged execution code, docs, and focused tests; no artifact migration, publication, credential storage, raw response persistence, release-gate change, provider-call authorization expansion, or MCP permission expansion is involved.
...
Implemented scope: human-baseline comparison now reports whether the candidate result includes an owner-baseline requirement contract matching the approved baseline; future advisory results retain the path-free owner-baseline requirement contract used during plan/run normalization; evidence-set comparison records preserve candidate owner-contract diagnostics; claim-standard-gate rerun plans add approval-required provider result targets when owner-baseline comparison cannot be fixed by local comparison regeneration alone; evidence regeneration planning verifies matching owner-baseline requirement contracts before resolving provider rerun commands; the Responses adapter startup output reports its effective upstream provider timeout; adapter upstream request failures include only allowlisted timeout/duration/failure-class diagnostics. Non-scope: automatic provider reruns, changing default timeout values, storing raw provider responses, storing credential values, exposing Agentic Human Review through MCP, authorizing human-equivalent or human-superior claims, mutating deterministic findings, changing release gates, parent-repository changes, consumer-repository changes, package publication, or CI-default live provider calls.

Implementation order: synchronize product and workflow documents; add owner-baseline contract diagnostics to comparison and evidence-set records; add provider-result repair targets to claim-standard-gate rerun planning; harden evidence regeneration approved-plan resolution so owner-baseline repair targets require matching owner-contract plans; add adapter timeout visibility and safe upstream failure details; extend focused no-browser CLI and adapter tests; run syntax checks, focused Agentic Human Review tests, product docs/security checks, and product gate before remote synchronization.

Recovery: this slice is additive and local. Existing Agentic Human Review proposal, plan, run, status/list, report-quality, benchmark, calibrate, compare, evidence-set, human-baseline lifecycle, xhigh, claim-readiness, longitudinal quality, claim-standard-gate, Responses adapter startup, visual review, agent execution, MCP profiles, safe HTTP MCP, release readiness, artifact-root policy, alias compatibility, deterministic findings, and release gates remain compatible. Rollback is a standard Git revert of the owner-baseline recovery diagnostics, adapter timeout diagnostics, docs, and focused tests; no artifact migration, publication, credential storage, raw response persistence, release-gate change, provider-call authorization, or MCP permission expansion is involved.

### Agentic Human Review Responses Adapter Provider Payload Compaction

Purpose: keep approved real-provider dogfood requests below the configured provider request-size cap after owner-baseline and xhigh proof contracts are attached, without raising size limits or weakening validation. The slice stays generic by deriving a compact provider-facing view from the request contracts, not from target URLs, repository names, page-specific literals, or one-off artifact paths.

Implemented scope: build a Responses-adapter-only compact `review_request` view that preserves plan hash, effort, roles, rounds, xhigh strict-output requirements, effective benchmark requirements, compact owner-baseline criteria, `required_benchmark_coverage`, `required_owner_baseline_findings`, `required_owner_baseline_coverage`, and catalog-backed evidence-reference ids; keep post-validation against the original inbound TraceCue request; remove duplicated long owner-baseline arrays, nested provider-instruction owner contracts, duplicated plan/package text and visual contracts, long owner summaries, and local paths from the provider-facing request; shorten evidence catalog descriptions; and emit non-secret request section byte counts when generated provider requests still exceed the configured cap. Non-scope: increasing request-size limits, accepting prose-only owner-baseline evidence, relaxing unknown evidence-reference rejection, synthesizing proof records from baseline text, changing provider execution approval, storing raw provider responses or credentials, exposing Agentic Human Review through MCP, mutating deterministic findings, changing release gates, parent-repository changes, consumer-repository changes, package publication, or CI-default live provider calls.

Implementation order: measure the failed dogfood request sections; add the compact provider-facing request view and size diagnostics; reduce duplicated owner-baseline instruction maps so providers use `input.required_owner_baseline_findings` and `input.required_owner_baseline_coverage`; add a real-shaped owner-baseline request-budget regression and extend the xhigh repair-budget regression; run syntax checks, focused Responses adapter tests, product docs/security checks, and product gate; then rerun the approved standard/deep/xhigh real-provider dogfood cells.

Recovery: this slice is additive and local. Existing Agentic Human Review proposal, plan, run, status/list, report-quality, benchmark, calibrate, compare, evidence-set, human-baseline lifecycle, xhigh, claim-readiness, longitudinal quality, claim-standard-gate, evidence regeneration planning, Responses adapter startup, visual review, agent execution, MCP profiles, safe HTTP MCP, release readiness, artifact-root policy, alias compatibility, deterministic findings, and release gates remain compatible. Rollback is a standard Git revert of the compact provider view, diagnostics, docs, and focused tests; no artifact migration, publication, credential storage, raw response persistence, release-gate change, provider-call authorization, or MCP permission expansion is involved.

### Agentic Human Review Owner Baseline Effective Coverage Contract Hardening

Purpose: make approved owner-baseline required mentions, required dimensions, and forbidden claims mechanically mandatory in provider output instead of relying on provider prompt interpretation or post-hoc prose matching. The slice keeps the design reusable across target artifacts by deriving obligations from the attached owner-baseline requirement contract and the request evidence catalog, not from product names, repository paths, target URLs, or one-off page text.

Implemented scope: derive compact `required_benchmark_coverage` templates from the effective benchmark coverage contract, derive compact `required_owner_baseline_coverage` templates from `owner_baseline_requirement_contract.required_mentions`, `required_dimensions`, and `forbidden_claims`; merge those values with benchmark-case requirements into the effective benchmark coverage contract; include the compact coverage templates in provider payloads and bounded repair context; add owner-baseline required mention, dimension, and forbidden-claim references to the adapter evidence catalog before optional artifact references; require exact matching `benchmark_requirement_coverage` records with catalog-backed evidence references; require owner-baseline forbidden claims to be structured absence rows; preserve the same effective coverage requirements in deterministic fake-provider coverage and provider instruction contracts; and add focused no-browser adapter tests for repairable owner-baseline coverage gaps. Non-scope: lowering coverage thresholds, accepting prose-only owner-baseline discussion as proof, synthesizing coverage records from baseline text, increasing repair attempts, authorizing human-equivalent or human-superior claims, storing raw provider responses or credentials, exposing Agentic Human Review through MCP, mutating deterministic findings, changing release gates, parent-repository changes, consumer-repository changes, package publication, or CI-default live provider calls.

Implementation order: synchronize product and workflow documents; add effective coverage helpers that combine benchmark and owner-baseline requirements; wire `required_owner_baseline_coverage` into provider input, adapter instructions, repair diagnostics, evidence-reference catalog priority, post-validation, and normalized result requirements; preserve effective coverage in the plan/provider instruction contract and deterministic fake-provider output; add focused adapter and planning tests; run syntax checks, focused Agentic Human Review tests, product docs/security checks, and product gate; then rerun only the approved real-provider dogfood cells needed to verify the repaired proof-quality path.

Recovery: this slice is additive and local. Existing Agentic Human Review proposal, plan, run, status/list, report-quality, benchmark, calibrate, compare, evidence-set, human-baseline lifecycle, xhigh, claim-readiness, longitudinal quality, claim-standard-gate, evidence regeneration planning, Responses adapter startup, visual review, agent execution, MCP profiles, safe HTTP MCP, release readiness, artifact-root policy, alias compatibility, deterministic findings, and release gates remain compatible. Rollback is a standard Git revert of the effective coverage, adapter validation, docs, and focused tests; no artifact migration, publication, credential storage, raw response persistence, release-gate change, provider call, or MCP permission expansion is involved.

### Agentic Human Review Evidence Regeneration Planning

Purpose: make failed owner-claim proof data recoverable without hand-written, target-specific instructions. The slice turns claim-standard-gate `rerun_plan` targets into a generic, dependency-ordered evidence regeneration plan that can be reused across target URLs, benchmark cases, repositories, and artifact layouts. It preserves the existing approval boundary: provider-result reruns remain command templates that require the normal approved plan-hash, exact transfer flags, and explicit execution path outside the planner.

Implemented scope: add `agentic review evidence-set regenerate plan --evidence-set <workspace-json> --claim-gate <workspace-json> [--target-registry <workspace-json>] --json`; accept evidence-set manifests, evidence-set summaries, and supported runtime envelopes; accept claim-standard-gate artifacts or runtime envelopes; resolve comparison inputs and owner-baseline comparison inputs from workspace-confined target registry data; resolve provider-rerun plan paths, plan hashes, and transfer flags only from validated approved-plan records or from the generic result -> execution -> plan artifact chain; validate plan artifact hashes, exact run commands, target case/effort alignment, and transfer flags before emitting a resolved provider-rerun command; fail closed with unresolved placeholders and warnings when registry or artifact-derived approved-plan data is missing, unreadable, mismatched, or tampered; classify result, claim-audit, calibration, comparison, and human-baseline comparison targets into dependency groups; emit command templates, invalidation metadata, downstream summary/claim-readiness/longitudinal-quality/claim-standard-gate commands, execution-boundary flags, and warnings for unresolved inputs; include missing calibration case-effort cells in the claim-standard-gate rerun plan; update schemas, API exports, CLI help, product/workflow/security/verification docs, manifests, packed-install coverage, and focused no-browser tests. Non-scope: running provider APIs, automatically executing reruns, writing regenerated artifacts, launching browsers, exposing the planner through MCP, weakening claim-standard-gate blockers, authorizing human-equivalent or human-superior claims, storing raw provider responses or credentials, mutating deterministic findings, changing release gates, parent-repository changes, consumer-repository changes, package publication, remote CI, or hard-coded target URL criteria.

Implementation order: synchronize product and workflow documents; fix claim-standard-gate minimal rerun target coverage for missing calibration cells; add the read-only evidence regeneration planner core; wire parser, CLI, public API, constants, schema registry, and public schema; add focused CLI tests for no-execute rejection, local-only calibration planning, provider-rerun approval-required planning, artifact-derived approved-plan resolution, explicit registry override, fail-closed plan/hash mismatch handling, downstream regeneration commands, and schema parity; update package smoke coverage and manifests; run syntax checks, focused Agentic Human Review tests, product docs/security checks, package smoke checks, and product gate; commit the completed local slice after checks pass.

Recovery: this slice is additive and read-only. Existing Agentic Human Review proposal, plan, run, status/list, report-quality, benchmark, calibrate, compare, evidence-set, human-baseline, xhigh, claim-readiness, longitudinal quality, claim-standard-gate, Responses adapter startup, visual review, agent execution, MCP profiles, safe HTTP MCP, release readiness, artifact-root policy, alias compatibility, deterministic findings, and release gates remain compatible. Rollback is a standard Git revert of the planner, schema, docs, and tests; no artifact migration, provider execution, credential storage, raw response persistence, release-gate change, or MCP permission expansion is involved.

### Agentic Human Review Proof-Claim Integrity And Minimal Rerun Planning

Purpose: make owner claim-review evidence safer to judge after real-provider dogfood by preventing unsupported review claims from entering strict proof eligibility and by turning failed claim-standard-gate output into a reusable minimal rerun plan. The slice keeps proof generation generic across target artifacts and benchmark cases: target-specific criteria come from owner-baseline contracts and evidence sets, while the implementation remains driven by schemas, normalized diagnostics, and command templates rather than target URLs or one-off fixtures.

Implemented scope: normalize advisory `review_claims` through a `claim_integrity` summary; reject placeholder, missing-text, missing-evidence, equality, and superiority claim text from strict claim-numerator eligibility; make the Responses adapter request, validate, repair, and normalize only catalog-backed or planned-role-backed review claims; propagate claim-integrity failures through evidence-set records, result proof eligibility, claim audit, and claim-standard-gate blockers; extend direct-vs-TraceCue and human-baseline comparisons with metric-level regression diagnostics, critical regressed metrics, missing owner labels, missing target-specific must-not-miss criteria, and evidence-backed forbidden-claim absence diagnostics; make forbidden-claim absence require local evidence references instead of text-only or count-only matches; add a read-only `rerun_plan` to claim-standard-gate output with minimal result, calibration, comparison, owner-baseline, xhigh, claim-audit, and readiness targets; update schemas, product/workflow/security/verification docs, manifests, and focused no-browser tests. Non-scope: provider execution approval, automatic rerun execution, artifact writes from the gate, MCP exposure, deterministic review mutation, release-gate mutation, human-equivalent or human-superior claim authorization, raw provider response storage, credential storage, parent-repository changes, consumer-repository changes, package publication, or target-specific hard-coded criteria.

Implementation order: synchronize product and workflow documents; add reusable claim-integrity normalization helpers; wire result proof eligibility, evidence-set records, claim audit, and claim-standard-gate failure details; harden adapter schema, instructions, validation, repair context, and normalization for review claims; add comparison metric diagnostics and human-baseline coverage diagnostics; add claim-standard-gate `rerun_plan` command templates without executing them; update schema registry and public schemas; extend focused Agentic Human Review CLI and adapter tests; run syntax checks, focused no-browser tests, product docs/security checks, package smoke checks, and product gate; commit the completed local slice after checks pass.

Recovery: this slice is additive and local. Existing Agentic Human Review proposal, plan, run, status/list, report-quality, benchmark, calibrate, compare, evidence-set, human-baseline, xhigh, claim-readiness, longitudinal quality, Responses adapter startup, visual review, agent execution, MCP profiles, safe HTTP MCP, release readiness, artifact-root policy, alias compatibility, deterministic findings, and release gates remain compatible. Rollback is a standard Git revert of the claim-integrity, comparison-diagnostic, adapter validation, schema, and rerun-plan changes; no artifact migration, publication, credential storage, raw response persistence, release gate change, provider call, or MCP permission expansion is involved.

### Agentic Human Review Proof-Readiness Normalization And Repair Diagnostics

Purpose: close the gap exposed by real-provider dogfood where evidence artifacts existed but could not be counted uniformly, and where xhigh/owner-baseline failures were too expensive to retry blindly. The slice makes proof-readiness diagnostics reusable across targets by normalizing supported API and CLI runtime-result artifacts, strengthening owner-baseline repair hints, and classifying claim-readiness blockers by cause.

Implemented scope: evidence-set artifact reading now unwraps supported TraceCue CLI and public API runtime-result wrappers for calibration, comparison, and human-baseline artifacts while preserving raw advisory-result validation for result entries; direct human-baseline comparison also accepts supported validation wrappers as the baseline input and revalidates the extracted baseline; evidence-set and claim-readiness summaries expose blocker categories for missing result cells, mechanical-contract incomplete cells, failed calibration cells, and missing comparison cells; the Responses adapter now requires owner label ids when owner-baseline criteria have owner labels, prioritizes required owner-baseline and benchmark catalog identifiers before optional artifact references, keeps provider instructions and external provider payloads size-bounded by using compact owner-baseline contract views instead of duplicating long owner-baseline and benchmark arrays, treats forbidden-claim `present=true` plus absence wording as a repairable semantic contradiction instead of proof, and sends repair attempts only redacted missing criterion ids, owner label ids, required fields, forbidden-claim semantics gaps, xhigh missing conditions, and allowed evidence-reference ids. Non-scope: weakening owner-baseline validation, accepting raw `agentic review run` runtime pointers as advisory result artifacts, authorizing human-equivalent or human-superior claims, storing raw provider responses, storing credentials, exposing Agentic Human Review through MCP, changing deterministic review output, mutating release gates, parent-repository changes, consumer-repository changes, package publication, or CI-default live provider calls.

Implementation order: synchronize `AGENTS.MD`, product and workflow documents, security, verification, handoff, tracker, and manifests; harden evidence-set runtime-result unwrapping; harden claim-readiness blocker classification; harden Responses adapter owner-label validation, repair context, and catalog priority; add no-provider regressions for API wrappers, run-wrapper rejection, blocker classification, owner-label repair, and catalog priority; run focused Agentic Human Review tests and product-local gates; then perform only bounded live dogfood reruns for remaining incomplete cells.

Recovery: this slice is additive and local. Existing Agentic Human Review proposal, plan, run, status/list, report-quality, benchmark, calibrate, compare, evidence-set, human-baseline, xhigh, claim-readiness, longitudinal quality, Responses adapter startup, visual review, agent execution, MCP profiles, safe HTTP MCP, release readiness, artifact-root policy, alias compatibility, deterministic findings, and release gates remain compatible. Rollback is a standard Git revert of the normalization, diagnostics, and adapter repair hardening changes; no artifact migration, publication, credential storage, raw response persistence, release gate change, or MCP permission expansion is involved.

### Agentic Human Review Owner Baseline Adapter Contract Repair Hardening

Purpose: make approved owner-baseline obligations easier for generic providers to satisfy mechanically without weakening the fail-closed owner-baseline validator. The slice addresses provider outputs that discuss owner-approved criteria in prose but do not return the exact structured criterion ids, owner label ids, and catalog-backed evidence references required for comparison-ready evidence.

Implemented scope: derive a compact `required_owner_baseline_findings` contract from the owner-baseline requirement contract and request evidence catalog; include it in provider payloads and bounded repair context; instruct providers to copy those ids into structured advisory findings; preserve recommended evidence-reference ids in repair diagnostics; normalize provider-authored structured findings from top-level advisory findings, owner-baseline finding arrays, and planned role findings; support conservative identifier aliases while still rejecting unknown owner labels or evidence references; and add focused no-browser regressions for role-level owner-baseline findings, alias normalization, repair payload templates, and free-text-only rejection. Non-scope: increasing repair attempts, synthesizing findings from baseline text, accepting unknown evidence references, weakening owner-baseline validation, authorizing human-equivalent or human-superior claims, storing raw provider responses, storing credentials, exposing Agentic Human Review through MCP, mutating deterministic findings, changing release gates, parent-repository changes, consumer-repository changes, package publication, or CI-default live provider calls.

Implementation order: synchronize product and workflow documents; add the required owner-baseline finding contract helper; wire the contract into provider input, instructions, repair context, schema hints, and diagnostics; broaden finding normalization only for provider-authored structured finding arrays; add focused adapter tests; run syntax checks, focused no-browser tests, product docs/security checks, and the product gate; then decide whether a single approved live dogfood rerun is warranted for the previously blocked cell.

Recovery: this slice is additive and local. Existing Agentic Human Review proposal, plan, run, status/list, report-quality, benchmark, calibrate, compare, evidence-set, human-baseline lifecycle, dogfood readiness, Responses adapter startup, visual review, agent execution, MCP profiles, safe HTTP MCP, release readiness, artifact-root policy, alias compatibility, deterministic findings, and release gates remain compatible. Rollback is a standard Git revert of the owner-baseline adapter contract hardening changes; no artifact migration, publication, credential storage, raw response persistence, release gate change, or MCP permission expansion is involved.

### Agentic Human Review Owner Baseline Contract Propagation

Purpose: make approved human-baseline criteria mechanically affect provider output and comparison evidence, instead of remaining an approval artifact that later reviews can ignore. The slice converts only verified owner-labeled human baselines into a path-free owner-baseline requirement contract that is bound into proposals, plans, plan hashes, provider payloads, Responses adapter validation, and human-baseline comparison matching.

Implemented scope: add `--human-baseline <owner-baseline-json>` to `agentic review propose` and `agentic review plan`; validate the input with the existing owner-labeled human-baseline verifier; fail closed for unapproved, synthetic, incomplete, or case-mismatched baselines; derive the benchmark case from the approved baseline when no explicit case is supplied; store a redacted owner-baseline requirement contract in plans, provider instruction contracts, review-quality benchmark contracts, result contracts, and generic-provider payloads without local evidence paths; require the Responses adapter to emit structured findings for every target-specific must-not-miss criterion using criterion ids, owner label ids, and catalog-backed evidence references; preserve those ids through advisory normalization; and let human-baseline comparison prefer structured ids and evidence-backed coverage records while keeping text-only matches diagnostic. Non-scope: automatic owner approval, generating owner labels without human review, provider execution approval, human-equivalent or human-superior claim authorization, raw provider response storage, credential storage, MCP exposure, deterministic review mutation, release-gate mutation, parent-repository changes, consumer-repository changes, package publication, or CI-default live provider calls.

Implementation order: synchronize `AGENTS.MD`, product docs, security, verification, handoff, and tracker; extend parser/CLI help; add owner-baseline contract loading and redaction; bind the contract into proposals, plans, plan hashes, provider payloads, and result contracts; harden Responses adapter instructions, schema, evidence catalog, validation, repair retries, and finding normalization; harden comparison matching with id-first structured findings and evidence-backed coverage fallback; add no-browser CLI/comparison/adapter regressions; then run focused Agentic Human Review tests and product-local gates before regenerating real-page dogfood evidence.

Recovery: this slice is additive and local. Existing Agentic Human Review proposal, plan, run, status/list, report-quality, benchmark, calibrate, compare, evidence-set, human-baseline lifecycle, dogfood readiness, Responses adapter startup, visual review, agent execution, MCP profiles, safe HTTP MCP, release readiness, artifact-root policy, alias compatibility, deterministic findings, and release gates remain compatible. Rollback is a standard Git revert of the owner-baseline contract propagation changes; no artifact migration, publication, credential storage, raw response persistence, release gate change, or MCP permission expansion is involved.

### Agentic Human Review Target-Specific Human Baseline Enforcement

Purpose: make owner-approved human-baseline approval fail closed when the target-specific review conditions are missing, instead of depending on an operating note or prompt protocol. The slice keeps reusable generic criteria, but requires the owner-approved packet to include target-specific must-not-miss criteria that are linked to evidence-backed owner labels before it can become comparison-ready owner evidence.

Implemented scope: approval packets now evaluate `must_not_miss_criteria` mechanically; an `approved` decision produces `owner_labeled: true` only when at least one target-specific criterion exists, every target-specific criterion is linked to a required owner label, and the linked owner label has local evidence references. Validation repeats the same check so hand-written baselines cannot bypass approval. Human-baseline comparison scores target-specific must-not-miss coverage separately and blocks ready-for-owner-review when a candidate result misses any target-specific criterion. Focused no-browser tests cover overlay-to-draft propagation, approval fail-closed behavior for generic-only criteria, owner-baseline validation, text-only comparison rejection, and must-not-miss comparison scoring. Non-scope: creating owner labels without human approval, authorizing equality or superiority claims, changing provider execution approval, storing raw provider responses, storing credentials, exposing Agentic Human Review through MCP, mutating deterministic findings, changing release gates, parent-repository changes, consumer-repository changes, package publication, or CI-default live provider calls.

Implementation order: synchronize product and workflow documents; harden human-baseline approval, validation, normalization, and comparison helpers; extend lifecycle tests with target-specific must-not-miss criteria and generic-only fail-closed coverage; run focused no-browser tests and product-local gates; then resume owner-approved real-page baseline comparison data collection.

Recovery: this slice is additive and local. Existing Agentic Human Review proposal, plan, run, status/list, report-quality, benchmark, calibrate, compare, evidence-set, human-baseline registry/overlay/draft, dogfood readiness, Responses adapter startup, visual review, agent execution, MCP profiles, safe HTTP MCP, release readiness, artifact-root policy, alias compatibility, deterministic findings, and release gates remain compatible. Rollback is a standard Git revert of the target-specific baseline enforcement changes; no artifact migration, publication, credential storage, raw response persistence, release gate change, or MCP permission expansion is involved.

### Agentic Human Review Proof Evidence Aggregation And Adapter Contract Hardening

Purpose: make real-provider dogfood evidence reusable across targets and benchmark cases without accepting provider-shaped ambiguity as proof. The slice fixes the generic evidence-set aggregation path for TraceCue CLI runtime-result artifacts and tightens the Responses adapter benchmark contract so forbidden-claim records and evidence references are interpreted mechanically and fail closed.

Implemented scope: unwrap supported calibration, comparison, and human-baseline CLI runtime-result artifacts inside evidence-set summaries while preserving raw advisory-result validation; require adapter benchmark records to carry non-empty evidence and catalog-backed evidence references; treat forbidden-claim `present` as claim presence rather than check coverage; require explicit absence confirmation for forbidden-claim records; reject unknown provider-supplied evidence-reference ids; keep repair retries bounded and replacement-only; and add no-browser regression coverage for wrapper aggregation, forbidden-claim absence normalization, and unknown-reference rejection. Non-scope: creating owner-approved baselines, completing the six-case direct-vs-TraceCue matrix, authorizing human-equivalent or human-superior claims, storing raw provider responses, storing credentials, exposing Agentic Human Review through MCP, mutating deterministic findings, changing release gates, parent-repository changes, consumer-repository changes, package publication, or CI-default live provider calls.

Implementation order: synchronize product and workflow documents; harden adapter schema, instructions, normalization, and validation; harden evidence-set artifact reading with explicit runtime-result data-key unwrapping; add focused no-browser regressions; run focused Agentic Human Review tests and product-local gates; then regenerate the real-page evidence set and continue owner-approved baseline and comparison data collection.

Recovery: this slice is additive and local. Existing Agentic Human Review proposal, plan, run, status/list, report-quality, benchmark, calibrate, compare, evidence-set, human-baseline, dogfood readiness, Responses adapter startup, visual review, agent execution, MCP profiles, safe HTTP MCP, release readiness, artifact-root policy, alias compatibility, deterministic findings, and release gates remain compatible. Rollback is a standard Git revert of the proof evidence aggregation and adapter contract hardening changes; no artifact migration, publication, credential storage, raw response persistence, release gate change, or MCP permission expansion is involved.

### Agentic Human Review Xhigh Mechanical Effort Enforcement

Purpose: make `xhigh` review quality observable and enforceable as a TraceCue contract instead of depending only on provider prompt wording or model-specific reasoning behavior. The slice keeps provider effort controls useful, but requires TraceCue-owned evidence that `xhigh` output actually contains the planned roles, rounds, critique, verification, synthesis, structured benchmark coverage, and local evidence-reference support before it can improve proof-readiness.

Implemented scope: add provider effort capability contracts with native effort binding metadata; record effort execution contracts, provider effort binding, strict output contracts, repair-retry readiness, and xhigh multi-step readiness in plans, API payloads, advisory results, and report-quality output; pass native Responses `reasoning.effort` when supported; validate adapter advisory JSON against required `xhigh` role/round/synthesis/placeholder/benchmark/evidence-reference rules before normalization; publish `agentic-human-review-xhigh-completion` schema metadata; propagate xhigh mechanical enforcement through xhigh planning/simulation, evidence-set summaries, human-baseline comparison, claim-readiness diagnostics, calibration, and longitudinal quality rollups; and exclude incomplete mechanical contracts from strict claim-numerator eligibility. Non-scope: claiming human equivalence or superiority, forcing provider-internal reasoning beyond public API controls, storing raw provider responses, storing credentials, expanding MCP exposure, changing deterministic review findings, mutating release gates, publishing packages, changing parent-repository files, or running live provider calls by default.

Implementation order: synchronize product, security, verification, handoff, tracker, package, and schema documentation; add provider effort capability and plan/payload contract fields; harden Responses adapter native effort binding and strict advisory validation; harden xhigh completion, evidence-set, comparison, claim-readiness, calibration, and longitudinal quality helpers; add schema/package/test coverage; run no-browser schema/API/adapter tests and product gates; then rerun real-page standard/deep/xhigh dogfood with approved owner baselines and compare evidence.
...
Implemented scope: build request-aware Responses adapter output guidance for benchmark-enabled requests; require exact benchmark coverage records, structured advisory findings, and evidence-reference identifiers in the adapter schema/instructions; canonicalize safe provider aliases for findings and benchmark records without fabricating positive coverage; preserve local evidence-reference identifiers only after provider output returns from the approved request context; replace count-only real-provider readiness with explicit benchmark-case by effort matrix reporting; add calibration-pass and case-level direct-vs-TraceCue blockers to claim-readiness diagnostics. Non-scope: generating owner-labeled baselines, treating AI drafts or synthetic/local fixtures as proof, permitting human-equivalent or human-superior claims, changing deterministic review output, changing release gates, storing raw provider responses, storing credential values, exposing Agentic Human Review through MCP, CI-default live provider calls, parent-repository changes, consumer-repository changes, or package publication.

Implementation order: synchronized product and workflow documents; inspected the current adapter and readiness helpers; hardened the adapter schema, instructions, evidence-reference catalog, and alias normalization; hardened evidence-set and claim-readiness matrix summaries; added focused no-browser tests for adapter coverage/evidence refs, matrix gaps, weak calibration blockers, and text-only owner-label insufficiency; run product docs, security, no-browser, package, and product gates; then rerun local evidence-set validation before any further live dogfood.

Recovery: this slice is additive and local. Existing Agentic Human Review proposal, plan, run, status/list, report-quality, benchmark, calibrate, compare, evidence-set, human-baseline, dogfood readiness, Responses adapter startup, visual review, agent execution, MCP profiles, safe HTTP MCP, release readiness, artifact-root policy, alias compatibility, deterministic findings, and release gates remain compatible. Rollback is a standard Git revert of the proof-readiness hardening changes; no artifact migration, publication, credential storage, raw response persistence, release gate change, or MCP permission expansion is involved.

### Agentic Human Review Proof-Policy Hardening And Evidence Eligibility

Purpose: make the human-baseline dogfood pipeline harder to misread as a completed human-equivalence proof. The slice separates synthetic/fake/injected/local validation fixtures from real-provider claim-numerator evidence, requires owner-label comparison scores to come from structured findings with local evidence references, and keeps claim flags false while exposing the remaining evidence gaps.

Implemented scope: clamp human-baseline rubric proof-policy fields to fail closed; keep approval packets from claiming proof authorization; reject synthetic, deterministic, fixture-only, or local-pipeline markers from owner-labeled baseline verification; add deterministic fake-provider structured findings with local evidence references for pipeline validation only; score owner-label coverage from evidence-backed structured findings; report ready human-baseline comparison coverage by benchmark case; expose evidence-set origin, provider-execution class, claim-numerator eligibility, and excluded-from-claim reason metadata; extend claim-readiness with a real-provider dogfood matrix prerequisite. Non-scope: live provider execution, CI-default provider calls, human-equivalent or human-superior claim authorization, MCP exposure, deterministic review mutation, release-gate mutation, credential reading, raw-provider-response storage, parent-repository changes, consumer-repository changes, or package publication.

Implementation order: synchronize product and workflow documents; harden human-baseline validation and comparison helpers; add fake-provider evidence-backed structured findings; extend evidence-set and claim-readiness summaries; add focused no-browser and packed-install regressions; run package, product, security, and repository gates; then continue real-provider standard/deep/xhigh dogfood with independent owner-approved baselines.

Recovery: this slice is additive and local. Existing Agentic Human Review proposal, plan, run, report-quality, benchmark, calibrate, compare, evidence-set, dogfood readiness, Responses adapter, visual review, agent execution, MCP profiles, safe HTTP MCP, release readiness, artifact-root policy, alias compatibility, deterministic findings, and release gates remain compatible. Rollback is a standard Git revert of the proof-policy hardening changes; no artifact migration, publication, credential storage, raw response persistence, release gate change, or MCP permission expansion is involved.

### Agentic Human Review Human Baseline Lifecycle And Claim Readiness

Purpose: make owner-labeled human review baselines reusable and operable without asking humans to rewrite every judgment for every target. The slice turns the session roadmap AHR-HB-01 through AHR-HB-10 into a local read-only lifecycle: reusable registry, case overlay, AI draft, owner approval packet, approved-baseline validation, direct-vs-TraceCue comparison classification, evidence-set integration, continuous rollup inputs, and claim-readiness checks.

Implemented scope: add `agentic review human-baseline registry|overlay|draft|approval|claim-readiness` on top of the existing `validate|compare` commands; require approval metadata before `owner_labeled: true` can verify; keep AI-generated drafts explicitly non-proof; classify baseline comparisons into matches, misses, over-reports, severity mismatches, and insufficient evidence; include ready human-baseline comparison counts in evidence-set summaries; expose schema registry, package schema files, API exports, parser/CLI help, no-browser tests, and packed-install coverage. Non-scope: live provider execution, automatic human approval, human-equivalent or human-superior claim authorization, MCP exposure, deterministic review mutation, release-gate mutation, credential reading, raw-provider-response storage, parent-repository changes, consumer-repository changes, or package publication.

Implementation order: synchronize product and workflow documents; add registry/overlay/draft/approval/claim-readiness runners; extend parser, CLI, API, schema registry, and schema files; harden human-baseline validation with approval metadata; extend comparison classification and evidence-set readiness; add focused no-browser and packed-install tests; run package, security, docs, and repository gates; then continue standard/deep/xhigh real-page dogfood with owner-approved baseline packets.

Recovery: this slice is additive and local. Existing Agentic Human Review proposal, plan, run, report-quality, benchmark, calibrate, compare, evidence-set, dogfood readiness, Responses adapter, visual review, agent execution, MCP profiles, safe HTTP MCP, release readiness, artifact-root policy, alias compatibility, deterministic findings, and release gates remain compatible. Rollback is a standard Git revert of the human-baseline lifecycle changes; no artifact migration, publication, credential storage, raw response persistence, release gate change, or MCP permission expansion is involved.
...
### Agentic Human Review Claim Standard Gate

Purpose: convert the owner claim-standard review boundary from an operational note into a mechanical, read-only gate. The slice lets local automation distinguish "diagnostics completed" from "owner claim-review readiness passed" without relying on prompt wording, session memory, or a human remembering to run separate checks.

Implemented scope: add `agentic review claim standard-gate --evidence-set <workspace-json> [--policy <workspace-json>] --json`; reuse evidence-set summary, human-baseline claim-readiness, longitudinal quality, comparison records, owner-baseline comparison records, xhigh completion, synthetic/local evidence exclusion, and per-result claim audits; classify blockers for incomplete matrices, weak calibration, missing comparison coverage, direct/provider/benchmark regressions, unverified or mismatched owner baselines, ineligible claim-numerator results, permissive raw policy input, and unsupported claim text; expose CLI/API/schema/package coverage; return a failed command status when the gate does not pass while keeping the output advisory-only and gate-effect-free. Non-scope: authorizing human-equivalent or human-superior claims, provider execution approval, MCP exposure, deterministic review mutation, release-gate mutation, artifact writes, raw provider response storage, credential persistence, parent-repository changes, consumer-repository changes, or package publication.

Implementation order: synchronize product and workflow documents; add the claim-standard-gate runner and fail-closed blocker builder; extend comparison records with owner-baseline match diagnostics; wire parser, CLI, API, schema registry, and public schema files; add no-browser CLI, architecture, schema parity, and packed-install coverage; run product document, security, package, and repository gates; then continue owner claim-standard review using the generated blocker output.

Recovery: this slice is additive and local. Existing Agentic Human Review proposal, plan, run, report-quality, benchmark, calibrate, compare, evidence-set, human-baseline, dogfood readiness, Responses adapter, visual review, agent execution, MCP profiles, safe HTTP MCP, release readiness, artifact-root policy, alias compatibility, deterministic findings, and release gates remain compatible. Rollback is a standard Git revert of the claim-standard-gate changes; no artifact migration, publication, credential storage, raw response persistence, release gate change, or MCP permission expansion is involved.
...
- AHR editorial-quality comparison adds `agentic review compare --comparison-kind editorial-quality` for local comparison between a workspace-confined reference review and a candidate editorial synthesis. It emits effort-target scores and diagnostics without provider calls, raw text output, source-text persistence, MCP exposure, proof-contract satisfaction, release-gate mutation, or equality/superiority claim authorization.
...
- Purpose: make real-page standard/deep/xhigh dogfood recovery repeatable when a provider follows the API effort setting but still omits TraceCue-required benchmark, owner-baseline, staged-role, or final effort proof records.
- Implemented scope: aggregate adapter contract failures across benchmark coverage, canonical `owner_baseline_findings`, optional review-claim filtering, and full or staged effort completion; build missing-only repair context with filtered required templates and recommended evidence ids; validate owner-baseline proof only from canonical target-specific owner-baseline records rather than role-level findings or prose; preserve `owner_baseline_findings` in normalized advisory results and merge them into comparison-visible `agentic_human_review_findings`; validate staged provider calls by current required roles/round and final-stage synthesis; persist safe staged and loopback adapter diagnostics in execution records and run receipts; expose adapter request/response size limits in CLI help and startup output.
- Non-scope: increasing default repair attempts, synthesizing proof records from prose, weakening owner-baseline or benchmark validation, treating intermediate stage outputs as final evidence, storing raw provider responses or request payloads, persisting credential values, adding target-specific branches, authorizing human-equivalent or human-superior claims, exposing Agentic Human Review through MCP, mutating deterministic findings, or changing release gates.
- Implementation order: repair adapter validation aggregation; add missing-only repair context helpers; preserve stage-aware effort validation; add allowlisted failure-diagnostic persistence in generic and staged provider paths; expose size-limit CLI metadata; add focused no-browser tests for aggregate repair, staged loopback diagnostics, existing owner-baseline repair behavior, and compact repair; synchronize product/security/verification/workflow docs; run syntax, focused tests, full local tests, product docs/security/structure checks, package smoke, product gate, and diff checks.
- Recovery: this slice is additive and local. Existing Agentic Human Review proposal, plan, run, report-quality, benchmark, calibrate, compare, evidence-set, human-baseline, dogfood readiness, Responses adapter, visual review, agent execution, MCP profiles, safe HTTP MCP, release readiness, artifact-root policy, alias compatibility, deterministic findings, and release gates remain compatible. Rollback is a standard Git revert of this slice; no artifact migration, publication, credential persistence, raw response persistence, release-gate change, or MCP permission expansion is involved.

## Agentic Human Review Owner-Baseline Label Obligation Enforcement

- Purpose: make owner-approved human-baseline comparison proof reusable across target artifacts by requiring every approved owner label, not only each target-specific criterion, to have canonical provider-authored evidence before claim-standard evidence can pass.
- Implemented scope: derive label-granular `required_owner_baseline_findings` obligations from the approved owner-baseline requirement contract; preserve target-specific criterion fallback obligations only when a criterion has no linked owner label; validate `owner_baseline_findings` against exact owner label ids, linked criterion ids, and catalog-backed evidence references; filter repair templates by missing owner label ids; prefer evidence-backed exact owner-label/criterion findings over earlier broad unbacked text matches during human-baseline comparison; and keep missing owner-label proof visible to claim-standard-gate.
- Non-scope: synthesizing owner-label proof from prose, benchmark coverage rows, coverage patch output, target URLs, case ids, target-specific branches, relaxed thresholds, role-level findings, generic advisory findings, raw artifacts, raw provider responses, request payloads, credentials, MCP exposure, deterministic review mutation, release-gate mutation, or human-equivalent/human-superior claim authorization.
- Implementation order: update adapter obligation derivation and post-validation; update human-baseline comparison candidate selection; add no-browser regressions for multi-label owner-baseline repair and masked evidence-backed matches; synchronize product and workflow documents; run syntax checks, focused human-baseline/Responses adapter tests, full local tests, product docs/security checks, product gate, and fresh downstream evidence regeneration from the latest real-provider artifacts.
- Recovery: this slice is additive and fail-closed. Existing artifacts can be re-compared locally when they already contain evidence-backed canonical owner-label findings; future provider runs that omit required owner labels remain repairable through the existing adapter retry path. Rollback is a standard Git revert of this slice and does not require artifact migration, credential cleanup, or policy changes.

## Agentic Human Review Editorial Synthesis

- Purpose: improve owner readability by adding a source-attributed editorial view over an existing normalized Agentic Human Review advisory result.
- Implemented scope: define optional `editorial_synthesis` result data; extract bounded text only from normalized result sections; resolve local language settings once during `agentic review run`; store bounded `language_settings` metadata; prefer resolved `artifact_output.language` for synthesis language metadata across the supported 14-locale contract; preserve source advisory text when translation execution is disabled; build one-sentence, full-review, observation, tension, strength, caution, direction, owner-decision, limitation, source-reference, and language-resolution fields; render the section and language metadata in Markdown reports; keep thin-input results limited instead of inventing richer review prose; and keep schema discoverability without making the field required for older artifacts.
- Non-scope: provider/API calls, subscription web UI automation, provider prompt/schema expansion, translation execution, source evidence translation, provider output translation, raw pixel reads, evidence transfer, credential reads, raw provider response storage, MCP exposure, deterministic finding mutation, report-quality metric mutation, owner-baseline proof satisfaction, benchmark coverage repair, claim-standard readiness, owner decision approval, release-gate mutation, or human-equivalent/human-superior claims.
- Implementation order: add the result contract and local synthesis builder; integrate the optional result field after normalization; wire local language settings into AHR normalization only; render Markdown from the result field; add focused tests for JSON, Markdown, source refs, 14-locale artifact-output language settings, unresolved fallback, no provider expansion, no additional API calls, no translation execution, and limited fallback behavior; synchronize product/security/verification/workflow docs; run syntax checks, focused AHR tests, full local tests, product docs/security checks, product gate, and diff checks.
- Recovery: this slice is additive and local. Existing results without `editorial_synthesis` remain valid, provider payloads do not request the field, and rollback is a standard Git revert without artifact migration, credential cleanup, policy changes, or remote service changes.

## Agentic Human Review Generic Content Evidence Intake

- Purpose: let Agentic Human Review distinguish page-only review from page-plus-content-evidence review for videos, web pages, PDFs, meeting notes, documents, transcripts, and other textual artifacts while keeping analyzers outside TraceCue and making TraceCue-owned report labels follow artifact output language settings.
- Implemented scope: accept optional `--content-evidence <workspace-json>` for `agentic review propose` and `agentic review plan`; keep `--video-evidence <workspace-json>` as a compatibility input; normalize analyzer-neutral bounded content evidence with source type, summaries, content units, claims, limitations, coverage, and provenance; reject raw media, raw binaries, base64/blob/data URI payloads, raw HTML/PDF bytes, full documents, and full transcripts while allowing explicit false privacy/boundary flags that declare raw/full content is absent; carry path-free supplemental content evidence into proposals, plans, packages, provider payload filters, advisory results, report-quality diagnostics, schemas, Markdown reports, and editorial synthesis; expose additive `content_evidence_*` fields on `evidence_scope`; let local editorial synthesis build content-evidence-first review paragraphs from bounded summaries, excerpt units, claims, limitations, and existing advisory signals before generic run summaries; classify bounded evidence density and content-review strength without changing scoring or gates; render localized source-type display labels while preserving canonical source-type values; explain source-text preservation when translation execution is disabled; reflect standard/deep/xhigh effort stance in the local synthesis without promoting non-xhigh output into verification; suppress template-only paragraphs and near-duplicate editorial sentences; allow provider payloads to receive bounded content evidence only under existing page-text transfer approval while stripping local paths and source locators; and resolve TraceCue-owned report headings, labels, fallback sentences, content-evidence labels, and evidence-scope lines through local report templates for the supported locale contract while preserving source evidence text when translation execution is disabled.
- Non-scope: implementing an analyzer, changing FrameCue, fetching or downloading remote content, analyzing raw video/documents inside TraceCue, sending raw media, raw documents, raw HTML/PDF bytes, or full transcripts to providers, adding provider/API calls, adding MCP tools, translating provider-authored text or source evidence, satisfying owner-baseline or benchmark proof, authorizing claim readiness, mutating deterministic findings, changing release gates, or adding target-specific branches for any site, repository, URL, provider, model, file path, or analyzer.
- Implementation order: add the schema and parser option; add workspace-confined content evidence loading and redaction; keep video evidence compatibility by projecting video summaries into generic supplemental content evidence; integrate source summaries and bounded units into proposal, plan, package, provider filter, advisory normalization, report-quality, Markdown report, and editorial synthesis; add content-evidence-first editorial composition that remains local, source-attributed, provider-free, duplicate-aware, and bounded by source material; add localized report template keys for content evidence, source-type display labels, density/review-strength guidance, source-text preservation explanation, and effort stance; add focused no-browser and packed-install tests for accepted false raw/full metadata, truthy raw/full metadata rejection, provider payload stripping, source-type matrix coverage through plan/run/report, evidence scope, source references, report-quality diagnostics, content-evidence-first full review text, source-text preservation, localized Markdown chrome, and canonical-value preservation; synchronize product/security/verification/workflow docs and manifests; run syntax checks, focused tests, full local tests, docs/security checks, product gate, and diff checks.
- Recovery: this slice is additive, bounded, and fail-closed. Existing proposals, plans, packages, advisory results, provider payloads, and Markdown reports remain valid when no content evidence is supplied. Rollback is a standard Git revert without artifact migration, credential cleanup, provider-policy change, MCP-policy change, or remote-service change.

## Agentic Human Review Source-Text Quality Owner Context Integration

- Purpose: make source-text effort-matrix quality diagnostics reusable by downstream owner-review surfaces without promoting the quality artifact into proof, warning, blocker, release-gate, rerun, or claim authority.
- Implemented scope: let evidence-set summaries accept workspace-confined source-text quality artifacts and supported CLI/API wrappers through an explicit artifact family; project a whitelist-only `owner_review_context.source_text_quality` into evidence-set, human-baseline claim-readiness, longitudinal-quality, claim-standard-gate, and evidence-regeneration outputs; include only counts, statuses, booleans, effort names, pass-condition summaries, output-safety summaries, context diagnostics, and regeneration freshness by effort; treat unreadable or invalid source-text quality artifacts as context diagnostics rather than evidence-set warnings; detect stale or missing effort artifacts for regeneration planning without emitting raw hash values; re-sanitize prebuilt evidence-set owner context before downstream propagation; neutralize unsafe provider/proof/claim/release/artifact-write/rerun authority flags; keep public schemas additive while typing optional `owner_review_context.source_text_quality`.
- Non-scope: changing source-text quality scoring, adding new CLI/MCP/provider surfaces, rerunning providers, writing artifacts from diagnostics, storing or exposing full source text, chunk text, candidate or reference prose, result paths, source locators, source titles, private source identity values, raw hashes, provider responses, credentials, proof contracts, warnings, blockers, claim-readiness conditions, passed states, equality or superiority claim states, release gates, future claim numerators, or automatic rerun execution.
- Implementation order: add the source-text quality evidence-set artifact family and wrapper handling; add local owner-context projection helpers beside the existing Agentic Human Review evidence-set code; thread optional context through claim-readiness, longitudinal-quality, claim-standard-gate, and evidence-regeneration planning; add focused no-browser regressions for baseline-vs-context gate neutrality, wrapper input, forged downstream context re-sanitization, unsafe authority-flag neutralization, unreadable artifact handling, misplaced result rejection, stale effort diagnostics, leak suppression, longitudinal propagation, and regeneration invalidation; synchronize product, workflow, security, verification, changelog, schema, and manifest records; run syntax checks, focused tests, full local tests, package smoke, product docs/security/structure checks, product gate, and diff checks.
- Recovery: this slice is additive and fail-closed. Existing evidence sets without source-text quality artifacts produce unchanged summaries, readiness, longitudinal, gate, and regeneration outputs. Rollback is a standard Git revert with no artifact migration, credential cleanup, provider-policy change, MCP-policy change, release-gate change, or claim-standard policy change.

## Control Center Browser Surface

- Purpose: add a minimal TraceCue-owned browser surface that helps non-engineers understand the current local review state without reading CLI JSON or running commands from the UI.
- Implemented scope: add `control-center status --json`, a body-free `control_center` read model, a loopback-only server with GET-only dashboard plus bounded local POST actions, a React + Vite app under `control-center/`, source-intake proposal creation from workspace-confined source text, display-language persistence to fixed TraceCue settings, product-local design-system token/component consumption from `docs/design-system/`, schema/API/parser/CLI coverage, server boundary tests, architecture tests, package file-set updates, and security allowlisting for the dedicated server/action modules.

...
### Agentic Human Review Staged Xhigh Provider Execution

Purpose: make approved real-provider `xhigh` dogfood recover from structural one-shot provider timeouts without relying on a target-specific retry, longer timeout, hard-coded URL, hard-coded model, or weaker validation. The slice remains generic by using the existing plan hash, transfer flags, provider capability hash, owner-baseline requirement contract, benchmark contract, and xhigh role/round plan as the execution authority.

Implemented scope: add an explicit `agentic review run --execution-mode staged` path for approved `xhigh` plans; preserve the default one-shot run path; split provider execution into bounded staged calls that keep env-only credentials, no raw provider response storage, no credential value recording, no MCP exposure, advisory-only output, and exact transfer boundaries; record normalized non-final stage metadata separately from final proof evidence; deterministically aggregate staged output into the existing `agentic_human_review_advisory` result contract; record true multi-call execution separately from xhigh completion, and mark xhigh complete or claim-ready only when the final normalized result satisfies the existing role, round, critique, verification, synthesis, benchmark, owner-baseline, and claim-integrity contracts. Non-scope: changing timeout defaults, treating stage output as final evidence, accepting incomplete xhigh as proof-ready, authorizing human-equivalent or human-superior claims, exposing Agentic Human Review through MCP, storing raw provider responses or credentials, mutating deterministic findings, changing release gates, parent-repository changes, consumer-repository changes, dependency additions, package publication, or target-specific dogfood branches.

Implementation order: synchronize product and workflow documents; add parser/API support for `--execution-mode staged` while keeping one-shot default; add reusable staged xhigh contract helpers and fail-closed validation for non-xhigh plans; implement staged execution for fake/injected/API providers through the existing provider and loopback adapter boundaries; add deterministic aggregation and execution/receipt metadata; extend focused no-browser tests for staged success, incomplete stage failure semantics, plan/transfer drift rejection before fetch, adapter no-store/no-tools boundaries, and claim-state non-authorization; run syntax checks, focused Agentic Human Review tests, product docs/security checks, package smoke checks, and product gate; then rerun the approved real-page staged dogfood cells only after local verification passes.
...
- Purpose: make real-page standard/deep/xhigh dogfood recovery repeatable when a provider follows the API effort setting but still omits TraceCue-required benchmark, owner-baseline, staged-role, or final `xhigh` proof records.
- Implemented scope: aggregate adapter contract failures across benchmark coverage, owner-baseline structured findings, optional review-claim filtering, and full or staged `xhigh` completion; build missing-only repair context with filtered required templates and recommended evidence ids; validate staged provider calls by current required roles/round and final-stage synthesis; persist safe staged and loopback adapter diagnostics in execution records and run receipts; expose adapter request/response size limits in CLI help and startup output.
...
- Implementation order: repair adapter validation aggregation; add missing-only repair context helpers; preserve stage-aware `xhigh` validation; add allowlisted failure-diagnostic persistence in generic and staged provider paths; expose size-limit CLI metadata; add focused no-browser tests for aggregate repair, staged loopback diagnostics, existing owner-baseline repair behavior, and xhigh compact repair; synchronize product/security/verification/workflow docs; run syntax, focused tests, full local tests, product docs/security/structure checks, package smoke, product gate, and diff checks.

...
Implemented scope: add `--source-text` to Agentic Human Review proposal and plan; normalize plain-text and JSON source-text exports; reject raw media, raw binaries, base64/blob/data URI payloads, credential-bearing fields, and raw/full structured payload fields; chunk and hash the text locally without persisting full source text or chunk text; derive an effort-aware `source_reading_review` with narrative flow, key points, concrete examples, tensions, cautions, recommended direction, excerpt refs, quality targets, and a natural review seed; include source-text metadata and source-reading review in plans, packages, provider filters, advisory results, editorial synthesis, Markdown reports, schemas, package smoke coverage, and CLI tests; and keep deterministic findings, metrics, release gates, MCP exposure, provider approval, owner-baseline proof, benchmark proof, equality claims, and superiority claims unchanged.
...
Implementation order: synchronize product/workflow/security/verification docs; add parser options and source-text normalization; add source-reading reviewer output and effort-depth behavior; integrate source reading into proposal, plan, review package, provider filters, adapter compact payload, result normalization, editorial synthesis, Markdown reports, schemas, schema registry, package-smoke coverage, and focused CLI tests; run syntax checks, full no-browser CLI tests, product docs/security checks, package checks, product gate, and `git diff --check`; then commit, push, monitor main CI, and synchronize local and remote.

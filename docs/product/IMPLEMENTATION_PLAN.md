...
Implemented scope: add `--source-text` to Agentic Human Review proposal and plan; normalize plain-text and JSON source-text exports; reject raw media, raw binaries, base64/blob/data URI payloads, credential-bearing fields, and raw/full structured payload fields; chunk and hash the text locally without persisting full source text or chunk text; derive an effort-aware `source_reading_review` with narrative flow, key points, concrete examples, tensions, cautions, recommended direction, excerpt refs, quality targets, and a natural review seed; derive an effort-aware `source_understanding_review` with thesis, audience promise, narrative arc, turning points, repeated motifs, must-not-miss points, limitations, reviewer implications, evidence claims, assistant-reference quality target, and coverage metrics; add an `editorial_integrator` record so editorial synthesis can use full-source understanding first and cross-check it against existing TraceCue findings, role opinions, owner-baseline findings, content evidence, video evidence, quality signals, and recommendations; compose the final review as natural owner-facing prose while keeping fake-provider scaffolding, `Step`/`role` markers, assistant-reference target labels, and boundary boilerplate out of the prose body; include source-text metadata, source-reading review, source-understanding review, editorial integration, source-understanding report-quality diagnostics, Markdown report sections, schemas, package smoke coverage, and CLI tests; and keep deterministic findings, release gates, MCP exposure, provider approval, owner-baseline proof, benchmark proof, equality claims, and superiority claims unchanged.

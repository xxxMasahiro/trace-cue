...
Implemented scope: build a Responses-adapter-only compact `review_request` view that preserves plan hash, effort, roles, rounds, xhigh strict-output requirements, effective benchmark requirements, compact owner-baseline criteria, `required_owner_baseline_findings`, `required_owner_baseline_coverage`, and catalog-backed evidence-reference ids; keep post-validation against the original inbound TraceCue request; remove duplicated long owner-baseline arrays, nested provider-instruction owner contracts, duplicated plan/package text and visual contracts, long owner summaries, and local paths from the provider-facing request; shorten evidence catalog descriptions; and emit non-secret request section byte counts when generated provider requests still exceed the configured cap. Non-scope: increasing request-size limits, accepting prose-only owner-baseline evidence, relaxing unknown evidence-reference rejection, synthesizing proof records from baseline text, changing provider execution approval, storing raw provider responses or credentials, exposing Agentic Human Review through MCP, mutating deterministic findings, changing release gates, parent-repository changes, consumer-repository changes, package publication, or CI-default live provider calls.

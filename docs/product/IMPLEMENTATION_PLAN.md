...
- Implemented scope: define optional `editorial_synthesis` result data; extract bounded text only from normalized result sections; build one-sentence, full-review, observation, tension, strength, caution, direction, owner-decision, limitation, and source-reference fields; render the section in Markdown reports; keep thin-input results limited instead of inventing richer review prose; and keep schema discoverability without making the field required for older artifacts.
- Non-scope: provider/API calls, subscription web UI automation, provider prompt/schema expansion, raw pixel reads, evidence transfer, credential reads, raw provider response storage, MCP exposure, deterministic finding mutation, report-quality metric mutation, owner-baseline proof satisfaction, benchmark coverage repair, claim-standard readiness, owner decision approval, release-gate mutation, or human-equivalent/human-superior claims.
- Implementation order: add the result contract and local synthesis builder; integrate the optional result field after normalization; render Markdown from the result field; add focused tests for JSON, Markdown, source refs, no provider expansion, no additional API calls, and limited fallback behavior; synchronize product/security/verification/workflow docs; run syntax checks, focused AHR tests, full local tests, product docs/security checks, product gate, and diff checks.

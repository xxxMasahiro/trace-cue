...
- Implementation order: add the schema and parser option; add workspace-confined content evidence loading and redaction; keep video evidence compatibility by projecting video summaries into generic supplemental content evidence; integrate source summaries and bounded units into proposal, plan, package, provider filter, advisory normalization, report-quality, Markdown report, and editorial synthesis; add content-evidence-first editorial composition that remains local, source-attributed, and provider-free; add localized report template keys for content evidence and xhigh stance; add focused no-browser and packed-install tests for accepted false raw/full metadata, raw-payload rejection, provider payload stripping, evidence scope, source references, report-quality diagnostics, content-evidence-first full review text, and localized Markdown chrome; synchronize product/security/verification/workflow docs and manifests; run syntax checks, focused tests, full local tests, docs/security checks, product gate, and diff checks.

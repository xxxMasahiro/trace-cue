...
- Keep agent advisory package, request-status, ingest, and report local, advisory-only, and separate from deterministic review findings, metrics, action plans, and release readiness.
...
- Keep `agent requests list` read-only over local package/result metadata; it must not launch browsers, execute agents, call providers, upload evidence, store credentials, mutate review artifacts, or expose MCP agent execution.

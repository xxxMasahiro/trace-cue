...
Responses adapter long-dogfood coverage must prove that the adapter reports its effective upstream provider timeout in startup output, documents that this timeout is separate from `AGENTIC_HUMAN_REVIEW_API_TIMEOUT_MS`, and returns only allowlisted failure diagnostics for upstream timeout or request failures. Adapter timeout diagnostics may include `duration_ms`, `timeout_ms`, `failure_class`, and safe cause/code values, but must not include credential values, stack traces, raw provider responses, provider request bodies, or credential-bearing endpoint strings.

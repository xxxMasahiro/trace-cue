...
Phase 29 adds no-browser coverage for `agent execution plan/run/status/list`, dry-run plan receipts, explicit `--execute` enforcement, fail-closed provider execution, execution schema parity, dashboard execution status fields, unchanged `agent_workflow` semantics, unchanged deterministic review gates, and MCP execution non-exposure. Future provider slices must add subscription runner abstraction, fake provider execution, API provider adapter boundaries, credential redaction, raw artifact non-transfer, raw provider response non-persistence, and advisory normalization checks before enabling execution.
...
- No-browser tests cover dry-run plans, local receipts, execution status/list aggregation, missing package behavior, fail-closed provider execution, and parser-level `--execute` enforcement. Future provider slices must add deterministic fake provider success/failure paths, missing credential behavior, and advisory-result normalization.

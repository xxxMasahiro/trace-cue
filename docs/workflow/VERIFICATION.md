...
- No-browser tests cover the generic source-type matrix for `video`, `web_page`, `pdf`, `meeting_notes`, `document`, `transcript`, and `other`, so content review is not tied to one analyzer, target site, URL, repository, provider, model, or file path.
...
- Report-template tests verify TraceCue-owned Agentic Human Review headings, labels, fallback text, composer connective text, and evidence-scope lines resolve through artifact output language settings while provider-authored advisory text and source evidence text remain preserved without translation execution.

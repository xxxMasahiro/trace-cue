...
- No-browser tests reject content evidence artifacts that contain raw media, raw binaries, base64/blob/data URI payloads, raw HTML/PDF bytes, full documents, or full transcripts, while allowing bounded `content_units` and explicit false-valued privacy or boundary metadata that declares raw/full content is absent.

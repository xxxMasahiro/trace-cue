...
- No-browser tests cover `agentic review quality source-text` over standard/deep/xhigh source-text result artifacts, including source-understanding completion, full-source/chunk-text non-persistence, pairwise effort deltas, xhigh critique readiness, optional reference-review comparison without prose leakage, schema/API/package coverage, and read-only `--execute` rejection.

...
- Purpose-led browser checks verify exactly three ordinary top-level destinations for `確認` (`confirm`), `進行中` (`running`), and `設定` (`settings`); five visible stages for `準備` (`prepare`), `確認` (`review`), `判断` (`decide`), `再確認` (`recheck`), and `完了` (`complete`); and continued reachability of Regression, Evidence, Findings, and Advanced through the details navigation on desktop and mobile.
...
- Compatibility checks compare the retained Regression review material and actions, Evidence matrix, Findings/blocker summaries, Advanced diagnostics, CLI output, and read-model fields before and after the ordinary navigation slice so simplification cannot remove existing capability.
...
- Vite build and browser smoke checks verify the React browser surface under `control-center/` can compile against `docs/design-system/tokens.json` and `docs/design-system/components.json`, render the Regression/Settings Playwright Test controls from built assets, show approved-CI policy fields, and keep `local_run` without a browser-run button.
